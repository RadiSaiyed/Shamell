...
import '../main.dart' show AppScaffold;
import 'app_shell_widgets.dart' show WaterButton;
...
class EquipmentRentalPage extends StatefulWidget {
  ...
  const EquipmentRentalPage(this.baseUrl,
      {super.key, this.walletId, this.operatorDomains});
...
class _EquipmentRentalPageState extends State<EquipmentRentalPage>
  ...
  bool get _isOperator =>
      (widget.operatorDomains ?? const []).contains('equipment');
  ...
  Future<void> _loadAssets() async {
    ...
    try {
      ...
      if (minPriceCtrl.text.trim().isNotEmpty)
        params['min_price'] = minPriceCtrl.text.trim();
      if (maxPriceCtrl.text.trim().isNotEmpty)
        params['max_price'] = maxPriceCtrl.text.trim();
      if (nearLatCtrl.text.trim().isNotEmpty)
        params['near_lat'] = nearLatCtrl.text.trim();
      if (nearLonCtrl.text.trim().isNotEmpty)
        params['near_lon'] = nearLonCtrl.text.trim();
      if (radiusCtrl.text.trim().isNotEmpty)
        params['max_distance_km'] = radiusCtrl.text.trim();
      if (minWeightCtrl.text.trim().isNotEmpty)
        params['min_weight'] = minWeightCtrl.text.trim();
      if (maxWeightCtrl.text.trim().isNotEmpty)
        params['max_weight'] = maxWeightCtrl.text.trim();
      if (minPowerCtrl.text.trim().isNotEmpty)
        params['min_power'] = minPowerCtrl.text.trim();
      if (maxPowerCtrl.text.trim().isNotEmpty)
        params['max_power'] = maxPowerCtrl.text.trim();
      if (tagsCtrl.text.trim().isNotEmpty) params['tag'] = tagsCtrl.text.trim();
      final uri =
          Uri.parse('$_base/equipment/assets').replace(queryParameters: params);
  ...
  Future<void> _loadBookings() async {
    ...
    try {
      ...
      final uri = Uri.parse('$_base/equipment/bookings')
  ...
  Future<void> _pickDates() async {
    try {
      ...
      final initialEnd =
          DateTime.tryParse(toCtrl.text) ?? now.add(const Duration(days: 1));
  ...
  Future<void> _pickPoint(String title, double? lat0, double? lon0,
      void Function(LatLng) onSelect) async {
  ...
  Future<List<dynamic>> _loadCalendar(int equipmentId) async {
    ...
    try {
      ...
      final uri = Uri.parse('$_base/equipment/calendar/$equipmentId')
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              Color c = available
                                  ...
                                  : (blocked ? Colors.orange : Colors.red);
                              return Container(
                                width: 72,
                      ...
                      if (days.isNotEmpty) ...[
                        ...
                        Wrap(
                          ...
                          children: days.map((e) {
                            ...
                            final bool booked = m['booked'] == true;
                            Color c;
                            if (available) {
                              c = Tokens.colorPayments.withValues(alpha: .8);
                            } else if (blocked) {
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                  'Depot: ${eqLat.toStringAsFixed(4)}, ${eqLon.toStringAsFixed(4)}',
  ...
  Future<void> _loadAnalytics() async {
    try {
      final r = await http.get(Uri.parse('$_base/equipment/analytics/summary'),
          headers: await _hdrEquipment());
      if (!mounted) return;
      if (r.statusCode == 200) {
        setState(
            () => _analytics = jsonDecode(r.body) as Map<String, dynamic>?);
      }
    } catch (_) {}
  }
  ...
  Future<void> _editAsset(Map<String, dynamic> asset) async {
    ...
    final cat =
        TextEditingController(text: asset['category']?.toString() ?? '');
    final subcat =
        TextEditingController(text: asset['subcategory']?.toString() ?? '');
    final daily = TextEditingController(
        text: asset['daily_rate_cents']?.toString() ?? '');
    final weekly = TextEditingController(
        text: asset['weekly_rate_cents']?.toString() ?? '');
    final monthly = TextEditingController(
        text: asset['monthly_rate_cents']?.toString() ?? '');
    final qtyCtrl =
        TextEditingController(text: asset['quantity']?.toString() ?? '1');
    final minDays =
        TextEditingController(text: asset['min_rental_days']?.toString() ?? '');
    final maxDays =
        TextEditingController(text: asset['max_rental_days']?.toString() ?? '');
    final weekendPct = TextEditingController(
        text: asset['weekend_surcharge_pct']?.toString() ?? '');
    final longtermPct = TextEditingController(
        text: asset['longterm_discount_pct']?.toString() ?? '');
    final minNotice = TextEditingController(
        text: asset['min_notice_hours']?.toString() ?? '');
    final specs = TextEditingController(text: asset['specs']?.toString() ?? '');
    final weight =
        TextEditingController(text: asset['weight_kg']?.toString() ?? '');
    final power =
        TextEditingController(text: asset['power_kw']?.toString() ?? '');
    final deposit =
        TextEditingController(text: asset['deposit_cents']?.toString() ?? '');
    final deliveryFee = TextEditingController(
        text: asset['delivery_fee_cents']?.toString() ?? '');
    final deliveryPerKm = TextEditingController(
        text: asset['delivery_per_km_cents']?.toString() ?? '');
    final imageUrl =
        TextEditingController(text: asset['image_url']?.toString() ?? '');
    final tags = TextEditingController(text: asset['tags']?.toString() ?? '');
    final lat =
        TextEditingController(text: asset['latitude']?.toString() ?? '');
    final lon =
        TextEditingController(text: asset['longitude']?.toString() ?? '');
    String out = '';
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
            ...
            try {
              final body = {
                ...
                'weekend_surcharge_pct':
                    double.tryParse(weekendPct.text.trim()),
                'longterm_discount_pct':
                    double.tryParse(longtermPct.text.trim()),
                ...
                'delivery_per_km_cents':
                    double.tryParse(deliveryPerKm.text.trim()),
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'Daily rate (cents)')),
                TextField(
                    controller: weekly,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Weekly rate (cents)')),
                TextField(
                    controller: monthly,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Monthly rate (cents)')),
                TextField(
                    controller: qtyCtrl,
                    keyboardType: TextInputType.number,
                ...
                TextField(
                    controller: specs,
                    decoration:
                        const InputDecoration(labelText: 'Specs (brief)')),
                TextField(
                    controller: weight,
                    keyboardType: TextInputType.number,
                ...
                TextField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration:
                        const InputDecoration(labelText: 'Min rental days')),
                TextField(
                    controller: maxDays,
                    keyboardType: TextInputType.number,
                    decoration:
                        const InputDecoration(labelText: 'Max rental days')),
                TextField(
                    controller: weekendPct,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Weekend surcharge %')),
                TextField(
                    controller: longtermPct,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Long-term discount %')),
                TextField(
                    controller: minNotice,
                    keyboardType: TextInputType.number,
                    decoration:
                        const InputDecoration(labelText: 'Min notice hours')),
                TextField(
                    controller: deposit,
                    keyboardType: TextInputType.number,
                    decoration:
                        const InputDecoration(labelText: 'Deposit (cents)')),
                TextField(
                    controller: deliveryFee,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Delivery base fee (cents)')),
                TextField(
                    controller: deliveryPerKm,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Delivery per km (cents)')),
                TextField(
                    controller: imageUrl,
          ...
          Future<void> submit() async {
            ...
            try {
              final body = {
                'title': title.text.trim(),
                'city': city.text.trim(),
                'category': cat.text.trim().isNotEmpty
                    ? cat.text.trim()
          ...
          return Column(
            ...
            children: [
              ...
              TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration:
                      const InputDecoration(labelText: 'Weekly rate (cents)')),
              TextField(
                  controller: monthly,
                  keyboardType: TextInputType.number,
                  decoration:
                      const InputDecoration(labelText: 'Monthly rate (cents)')),
              TextField(
                  controller: qtyCtrl,
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(labelText: 'Quantity')),
              ...
              TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration:
                      const InputDecoration(labelText: 'Weekend surcharge %')),
              TextField(
                  controller: longtermPct,
                  keyboardType: TextInputType.number,
                  decoration:
                      const InputDecoration(labelText: 'Long-term discount %')),
              TextField(
                  controller: minNotice,
                  keyboardType: TextInputType.number,
                  decoration:
  ...
  Future<void> _quoteAndBook(Map<String, dynamic> asset) async {
    ...
    Future<void> submit() async {
      try {
        ...
        final payload = {
          ...
          'delivery_address': deliveryAddress.text.trim().isEmpty
              ? null
              : deliveryAddress.text.trim(),
          'pickup_address': pickupAddress.text.trim().isEmpty
              ? null
    ...
    Future<void> geocodeAddress(
        TextEditingController ctrl, bool isDelivery) async {
      try {
        final q = ctrl.text.trim();
        if (q.isEmpty) return;
        final uri =
            Uri.parse('$_base/osm/geocode').replace(queryParameters: {'q': q});
          ...
          if (body is List && body.isNotEmpty) {
            ...
            setState(() {
              _addressSuggestions = (body as List)
                  .take(5)
                  .map((e) =>
                      (e as Map)['display_name']?.toString() ??
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                          ...
                          controller: from,
                          decoration:
                              const InputDecoration(labelText: 'From (ISO)'),
                        )),
                        const SizedBox(width: 8),
                        Expanded(
                            child: TextField(
                          controller: to,
                          decoration:
                              const InputDecoration(labelText: 'To (ISO)'),
                        )),
                      ],
                    ),
                    Row(
                    ...
                    if (delivery) ...[
                      ...
                      Row(
                        children: [
                          ...
                          IconButton(
                              onPressed: () =>
                                  geocodeAddress(pickupAddress, false),
                              icon: const Icon(Icons.search))
                        ],
                      ),
                      Row(
                        children: [
                          Expanded(
                              child: Text(
                            _deliveryLat != null
                                ? 'Dropoff: ${_deliveryLat?.toStringAsFixed(5)}, ${_deliveryLon?.toStringAsFixed(5)}'
                                : 'Dropoff pin not set',
                            style: const TextStyle(fontSize: 12),
                          )),
                          TextButton(
                      ...
                      Row(
                        children: [
                          Expanded(
                              child: Text(
                            _pickupLat != null
                                ? 'Pickup: ${_pickupLat?.toStringAsFixed(5)}, ${_pickupLon?.toStringAsFixed(5)}'
                                : 'Pickup pin not set',
                            style: const TextStyle(fontSize: 12),
                          )),
                          TextButton(
                    ...
                    if (calendar.isNotEmpty)
                      SizedBox(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            final d =
                                calendar[i] as Map<String, dynamic>? ?? {};
                            final available = d['available'] == true;
                            final blocked = d['blocked'] == true;
                            Color c = available
                                ? Tokens.colorPayments
                              ...
                              onTap: available
                                  ? () {
                                      ...
                                      if (day != null) {
                                        ...
                                        from.text = fmt.format(DateTime(
                                                day.year, day.month, day.day, 8)
                                            .toUtc());
                                        to.text = fmt.format(DateTime(day.year,
                                                day.month, day.day, 18)
                                            .toUtc());
                                        setSheet(() {});
                                      }
                                    }
                    ...
                    if (delivery)
                      TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Delivery distance (km, optional)'),
                      ),
                    TextField(
                      controller: notes,
                      decoration:
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text('Quote breakdown',
                                style: TextStyle(fontWeight: FontWeight.w700)),
                            Text(
                                'Rental: ${quotePayload?['rental_cents'] ?? ''}'),
                            Text(
                                'Delivery: ${quotePayload?['delivery_cents'] ?? ''}'),
                            Text(
                                'Deposit: ${quotePayload?['deposit_cents'] ?? ''}'),
                            if (quotePayload?['weekend_days'] != null)
                              Text(
                                  'Weekend days: ${quotePayload?['weekend_days']}'),
                            if (quotePayload?['min_days_applied'] != null)
                              Text(
                                  'Min days: ${quotePayload?['min_days_applied']}'),
                            if (quotePayload?['longterm_discount_pct'] != null)
                              Text(
                                  'Long-term discount: ${quotePayload?['longterm_discount_pct']}%'),
                            if (quotePayload?['delivery_distance_km'] != null)
                              Text(
                                  'Route: ${quotePayload?['delivery_distance_km']} km'),
                            Text(
                                'Total: ${quotePayload?['total_cents'] ?? ''} ${quotePayload?['currency'] ?? ''}'),
                          ],
                        ),
                      ),
                    if (quotePayload?['delivery_route'] is List &&
                        eqLat != null &&
                        eqLon != null)
                      Padding(
                        padding: const EdgeInsets.only(bottom: 12),
                        child: SizedBox(
                          ...
                          child: ShamellMapWidget(
                            center: LatLng(
                                _deliveryLat ?? eqLat, _deliveryLon ?? eqLon),
                            initialZoom: 11,
                            markers: {
                              Marker(
                                  markerId: const MarkerId('eq'),
                                  position: LatLng(eqLat, eqLon)),
                              ...
                              if (_pickupLat != null && _pickupLon != null)
                                Marker(
                                    ...
                                    position: LatLng(_pickupLat!, _pickupLon!)),
                            },
                            polylines: {
                              Polyline(
                                ...
                                width: 4,
                                points:
                                    (quotePayload!['delivery_route'] as List)
                                        .map((p) => LatLng(
                                              (p[0] as num).toDouble(),
                                              (p[1] as num).toDouble(),
                                            ))
                  ...
                  child: GlassPanel(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                            '${a['city'] ?? ''} • ${a['category'] ?? ''}${a['subcategory'] != null ? ' / ${a['subcategory']}' : ''}',
                            style: const TextStyle(
                                fontSize: 12, color: Colors.white70)),
                        if (dist != null)
                          Text('~${dist.toString()} km away',
                              style: const TextStyle(
                                  fontSize: 12, color: Colors.white70)),
                        if ((a['weight_kg'] ?? '') != '')
                          Text('Weight: ${a['weight_kg']} kg',
                              style: const TextStyle(
                                  fontSize: 12, color: Colors.white70)),
                        if ((a['power_kw'] ?? '') != '')
                          Text('Power: ${a['power_kw']} kW',
                              style: const TextStyle(
                                  fontSize: 12, color: Colors.white70)),
                        if ((a['tags'] ?? '') != '')
                          Text('Tags: ${a['tags']}',
                        ...
                        Row(
                          children: [
                            ...
                            if (_isOperator)
                              TextButton(
                                  onPressed: () => _addHold(a),
                                  child: const Text('Hold/maintenance')),
                            Expanded(
                                child: WaterButton(
                              label: 'Book',
                ...
                return GlassPanel(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text('From: ${b['from_iso'] ?? ''}',
                          style: const TextStyle(fontSize: 12)),
                      Text('To: ${b['to_iso'] ?? ''}',
                          style: const TextStyle(fontSize: 12)),
                      Text('Qty: ${b['quantity'] ?? ''}',
                          style: const TextStyle(fontSize: 12)),
                      Text('Amount: $amount ${b['currency'] ?? ''}',
                          style: const TextStyle(fontSize: 12)),
                      if (_isOperator)
                        Row(
                          children: [
                            TextButton(
                                onPressed: () => _updateStatus(
                                    b['id']?.toString() ?? '', 'confirmed'),
                                child: const Text('Confirm')),
                            TextButton(
                                onPressed: () => _updateStatus(
                                    b['id']?.toString() ?? '', 'active'),
                                child: const Text('Start')),
                            TextButton(
                                onPressed: () => _updateStatus(
                                    b['id']?.toString() ?? '', 'completed'),
        ...
        children: [
          Row(
            children: [
              Expanded(
                  child: WaterButton(
                      label: 'New asset', onTap: _createAssetDialog)),
              const SizedBox(width: 12),
              IconButton(
                  onPressed: _loadAnalytics, icon: const Icon(Icons.refresh)),
            ],
          ),
          const SizedBox(height: 12),
          _buildAnalytics(),
                      ...
                      children: [
                        Text('${t['kind']} • ${t['status']}',
                            style:
                                const TextStyle(fontWeight: FontWeight.w600)),
                        Text(
                            'Booking ${t['booking_id']} • Eq ${t['equipment_id']}',
                            style: const TextStyle(fontSize: 12)),
                        if (t['address'] != null)
