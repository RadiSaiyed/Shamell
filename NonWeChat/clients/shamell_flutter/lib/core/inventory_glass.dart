...
class _InventoryGlassPageState extends State<InventoryGlassPage>
  ...
  final _price =
      TextEditingController(text: '0'); // purchase price per unit (cents)
    ...
    try {
      ...
      if (r.statusCode == 200) {
        ...
        for (final i in arr) {
          ...
          if (i['low_stock_threshold'] != null &&
              qty <= (i['low_stock_threshold'] ?? 0)) {
            low.add(i);
          }
        }
        setState(() {
  ...
  Future<void> _create() async {
    ...
    try {
      ...
      final r = await http.post(Uri.parse('$_base/pos/inventory/items'),
          headers: await _hdrInv(json: true), body: body);
  ...
  Future<void> _adjust(int id, double delta) async {
    try {
      final body = jsonEncode(
          {"inventory_item_id": id, "qty": delta, "reason": "adjustment"});
      await http.post(Uri.parse('$_base/pos/inventory/movements'),
          headers: await _hdrInv(json: true), body: body);
      await _load();
    } catch (_) {}
  }
        ...
        children: [
          ...
          ListView(
            ...
            children: [
                        ...
                        SizedBox(
                            ...
                            child: TextField(
                                ...
                                decoration: const InputDecoration(
                                    labelText: 'Purchase price (cents)'),
                                keyboardType: TextInputType.number)),
                        SizedBox(
                            width: 140,
                            child: TextField(
              ...
              GlassPanel(
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    Chip(
                        label: Text(
                            'Total stock value: ${_totalCost.toStringAsFixed(2)}')),
              ...
              const SizedBox(height: 12),
              ..._items.map((i) {
                final low = i['low_stock_threshold'] != null &&
                    (i['stock_qty'] as num) <= (i['low_stock_threshold'] ?? 0);
                return GlassPanel(
                  padding: const EdgeInsets.all(12),
                  child: Row(
                    children: [
