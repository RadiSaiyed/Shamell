...
class _CarrentalModernPageState extends State<CarrentalModernPage> {
  ...
  Future<void> _persistSaved() async {
    ...
    await sp.setStringList(
        'carrental_saved', _saved.map((e) => e.toString()).toList());
  }
          ...
          return Padding(
            ...
            child: StatefulBuilder(builder: (context, setSheet) {
              Future<void> submit() async {
                ...
                try {
                  final resp =
                      await http.post(Uri.parse('$_base/carrental/book'),
                          headers: await _hdrCarrental(json: true),
                          body: jsonEncode({
                            'car_id': car['id'],
                            'renter_name': nameCtrl.text.trim(),
                            'renter_phone': phoneCtrl.text.trim().isEmpty
                                ? null
                                : phoneCtrl.text.trim(),
                    ...
                    children: [
                      ...
                      DropdownButton<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'standard',
                              child: Text('Standard (included)')),
                          DropdownMenuItem(
                              value: 'plus', child: Text('Plus (+15%)')),
                        ],
                        onChanged: (v) {
                          if (v == null) return;
  ...
  Future<void> _loadBookings() async {
    ...
    try {
      ...
      final uri = Uri.parse('$_base/carrental/bookings')
  ...
  Widget build(BuildContext context) {
        ...
        children: [
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            ChoiceChip(
              ...
              onSelected: (_) => setState(() => _transmissionFilter =
                  _transmissionFilter == 'auto' ? '' : 'auto'),
            ),
            ChoiceChip(
              label: const Text('Manual'),
              selected: _transmissionFilter == 'manual',
              onSelected: (_) => setState(() => _transmissionFilter =
                  _transmissionFilter == 'manual' ? '' : 'manual'),
            ),
            ChoiceChip(
              label: const Text('Electric'),
              selected: _fuelFilter == 'electric',
              onSelected: (_) => setState(() =>
                  _fuelFilter = _fuelFilter == 'electric' ? '' : 'electric'),
            ),
            ChoiceChip(
              label: const Text('Hybrid'),
              selected: _fuelFilter == 'hybrid',
              onSelected: (_) => setState(
                  () => _fuelFilter = _fuelFilter == 'hybrid' ? '' : 'hybrid'),
            ),
            ChoiceChip(
            ...
            ChoiceChip(
              ...
              onSelected: (_) => setState(() =>
                  _classFilter = _classFilter == 'economy' ? '' : 'economy'),
            ),
            ChoiceChip(
              label: const Text('SUV'),
              selected: _classFilter == 'suv',
              onSelected: (_) => setState(
                  () => _classFilter = _classFilter == 'suv' ? '' : 'suv'),
            ),
            ChoiceChip(
              label: const Text('Premium'),
              selected: _classFilter == 'premium',
              onSelected: (_) => setState(() =>
                  _classFilter = _classFilter == 'premium' ? '' : 'premium'),
            ),
            SizedBox(
              width: 160,
              child: TextField(
            ...
            SizedBox(
              ...
              child: TextField(
                ...
                keyboardType: TextInputType.number,
                onChanged: (v) =>
                    setState(() => _minSeats = int.tryParse(v) ?? 0),
              ),
            ),
            FilterChip(
    ...
    List<Map<String, dynamic>> _filteredCars() {
      ...
      final filtered = list.where((c) {
        ...
        final price = (c['price_per_day_cents'] ?? 0) is num
            ? (c['price_per_day_cents'] as num) / 100
            : 0;
        ...
        if (_freeCancelOnly && (c['free_cancel'] ?? false) != true)
          return false;
        if (_unlimitedOnly && (c['unlimited_mileage'] ?? false) != true)
          return false;
        final pickup = (c['city'] ?? '').toString();
        final drop = (c['dropoff_city'] ?? '').toString();
        if (_oneWayOnly) {
          if (drop.isEmpty || drop.toLowerCase() == pickup.toLowerCase())
            return false;
        }
        return true;
      }).toList();
    ...
    Widget carCard(Map<String, dynamic> car) {
      ...
      final isOneWay =
          drop.isNotEmpty && drop.toLowerCase() != city.toLowerCase();
      return GlassPanel(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                if (trips.isNotEmpty)
                  Chip(
                      label: Text('$trips trips'),
                      visualDensity: VisualDensity.compact),
                if (cls.isNotEmpty)
                  Chip(label: Text(cls), visualDensity: VisualDensity.compact),
                if (isOneWay)
                  Chip(
            ...
            Wrap(spacing: 8, runSpacing: 4, children: [
              if (seats.isNotEmpty)
                Chip(
                    ...
                    visualDensity: VisualDensity.compact),
              if (trans.isNotEmpty)
                Chip(label: Text(trans), visualDensity: VisualDensity.compact),
              if (fuel.isNotEmpty)
                Chip(label: Text(fuel), visualDensity: VisualDensity.compact),
              if ((car['free_cancel'] ?? false) == true)
                Chip(
                  label: const Text('Free cancel'),
                  visualDensity: VisualDensity.compact,
                  backgroundColor:
                      Tokens.colorAgricultureLivestock.withValues(alpha: .1),
                ),
              if (unlimited)
                Chip(
                  label: const Text('Unlimited mileage'),
                  visualDensity: VisualDensity.compact,
                  backgroundColor:
                      Tokens.colorCourierTransport.withValues(alpha: .12),
                ),
            ]),
            const SizedBox(height: 6),
            Text(
            ...
            if (_durationDays > 0)
              Text(
                'Est. total ${(_durationDays * price).toStringAsFixed(2)} $currency',
                style: theme.textTheme.bodySmall?.copyWith(
                  color: theme.colorScheme.onSurface.withValues(alpha: .70),
                ),
              ),
    ...
    if (filtered.isNotEmpty) {
      final totals = filtered
          .map((c) => ((c['price_per_day_cents'] ?? 0) as num? ?? 0))
          .toList();
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              IconButton(
                  onPressed: _loadBookings, icon: const Icon(Icons.refresh)),
            ],
          ),
          const SizedBox(height: 8),
          Wrap(
          ...
          ListView.separated(
            ...
            itemBuilder: (_, idx) {
              ...
              return ListTile(
                title:
                    Text('Booking #$id — ${(b['car_name'] ?? '').toString()}'),
                subtitle: Text(
        ...
        Wrap(
          ...
          children: [
            ...
            if (fromCtrl.text.isNotEmpty && toCtrl.text.isNotEmpty)
              Chip(
                label: Text(
                    'Dates: ${fromCtrl.text} → ${toCtrl.text} (${_durationDays}d)'),
