...
class _AgriMarketplacePageState extends State<AgriMarketplacePage>
  ...
  void initState() {
    super.initState();
    _tab = TabController(length: kEnduserOnly ? 1 : 2, vsync: this);
    _loadListings();
    if (!kEnduserOnly) {
      _loadOps();
    }
  ...
  Future<void> _loadListings() async {
    ...
    final uri = Uri.parse('${widget.baseUrl}/agriculture/listings')
    ...
    try {
      ...
      if (r.statusCode == 200) {
        ...
        if (body is List) {
          listings =
              body.map<Map<String, dynamic>>((e) => (e as Map).cast()).toList();
    ...
    try {
      final r = await http.post(Uri.parse('${widget.baseUrl}/agriculture/rfqs'),
          ...
          body: jsonEncode({
            ...
            'unit': rfqUnitCtrl.text.trim().isEmpty
                ? null
                : rfqUnitCtrl.text.trim(),
            'city': rfqCityCtrl.text.trim().isEmpty
                ? null
                : rfqCityCtrl.text.trim(),
            'target_date': rfqDateCtrl.text.trim().isEmpty
                ? null
                : rfqDateCtrl.text.trim(),
            'notes': rfqNotesCtrl.text.trim().isEmpty
                ? null
                : rfqNotesCtrl.text.trim(),
            'buyer_wallet_id': rfqBuyerCtrl.text.trim().isEmpty
                ? null
                : rfqBuyerCtrl.text.trim(),
          }));
    ...
    try {
      ...
      final ordParams = {
        ...
        if (orderStatusFilterCtrl.text.isNotEmpty)
          'status': orderStatusFilterCtrl.text.trim(),
      };
      final r1 = await http.get(
          Uri.parse('${widget.baseUrl}/agriculture/rfqs')
              .replace(queryParameters: rfqParams),
          headers: await _agriHeaders());
      final r2 = await http.get(
          Uri.parse('${widget.baseUrl}/agriculture/orders')
              .replace(queryParameters: ordParams),
          headers: await _agriHeaders());
  ...
  Future<void> _loadReplies(int rid) async {
    try {
      final r = await http.get(
          Uri.parse('${widget.baseUrl}/agriculture/rfqs/$rid/replies'),
          headers: await _agriHeaders());
      if (r.statusCode == 200) {
        final body = jsonDecode(r.body);
        if (body is List) {
  ...
  Future<void> _createListing() async {
    ...
    try {
      final r = await http.post(
          Uri.parse('${widget.baseUrl}/agriculture/listings'),
          headers: await _agriHeaders(json: true),
          body: jsonEncode({
            'title': lTitleCtrl.text.trim(),
            'category':
                lCatCtrl.text.trim().isEmpty ? null : lCatCtrl.text.trim(),
            'variety':
                lVarCtrl.text.trim().isEmpty ? null : lVarCtrl.text.trim(),
            'price_cents': int.tryParse(lPriceCtrl.text.trim()) ?? 0,
            'city':
                lCityCtrl.text.trim().isEmpty ? null : lCityCtrl.text.trim(),
            'origin': lOriginCtrl.text.trim().isEmpty
                ? null
                : lOriginCtrl.text.trim(),
            'certifications':
                lCertCtrl.text.trim().isEmpty ? null : lCertCtrl.text.trim(),
            'min_qty': double.tryParse(lMinCtrl.text.trim()),
            'unit':
                lUnitCtrl.text.trim().isEmpty ? null : lUnitCtrl.text.trim(),
            'status': lStatusCtrl.text.trim().isEmpty
                ? null
                : lStatusCtrl.text.trim(),
            'seller_wallet_id': lSellerCtrl.text.trim().isEmpty
                ? null
                : lSellerCtrl.text.trim(),
          }));
  ...
  Future<void> _updateListingStatus(int id, String status) async {
    ...
    try {
      final r = await http.patch(
          Uri.parse('${widget.baseUrl}/agriculture/listings/$id'),
          headers: await _agriHeaders(json: true),
  ...
  Future<void> _updateListingPrice(int id, int price) async {
    ...
    try {
      final r = await http.patch(
          Uri.parse('${widget.baseUrl}/agriculture/listings/$id'),
          headers: await _agriHeaders(json: true),
          body: jsonEncode({'price_cents': price}));
      setState(() => listingOut = '${r.statusCode}: ${r.body}');
      _loadListings();
    } catch (e) {
      setState(() => listingOut = 'error: $e');
  ...
  Future<void> _replyRFQ(int rid) async {
    ...
    try {
      final r = await http.post(
          Uri.parse('${widget.baseUrl}/agriculture/rfqs/$rid/reply'),
          headers: await _agriHeaders(json: true),
          body: jsonEncode({
            'price_per_unit_cents':
                int.tryParse(replyPriceCtrl.text.trim()) ?? 0,
            'eta_days': int.tryParse(replyEtaCtrl.text.trim()),
            'message': replyMsgCtrl.text.trim().isEmpty
                ? null
                : replyMsgCtrl.text.trim(),
            'supplier_wallet_id': replyWalletCtrl.text.trim().isEmpty
                ? null
                : replyWalletCtrl.text.trim(),
          }));
  ...
  Future<void> _createOrderFromListing(int lid) async {
    ...
    try {
      final r =
          await http.post(Uri.parse('${widget.baseUrl}/agriculture/orders'),
              headers: await _agriHeaders(json: true),
              body: jsonEncode({
                'listing_id': lid,
                'quantity': double.tryParse(orderQtyCtrl.text.trim()),
                'unit': orderUnitCtrl.text.trim().isEmpty
                    ? null
                    : orderUnitCtrl.text.trim(),
                'buyer_wallet_id': orderBuyerCtrl.text.trim().isEmpty
                    ? null
                    : orderBuyerCtrl.text.trim(),
                'seller_wallet_id': orderSellerCtrl.text.trim().isEmpty
                    ? null
                    : orderSellerCtrl.text.trim(),
                'price_cents': int.tryParse(lPriceCtrl.text.trim()) ?? 0,
                'buyer_notes': orderBuyerNotesCtrl.text.trim().isEmpty
                    ? null
                    : orderBuyerNotesCtrl.text.trim(),
                'supplier_notes': orderSupplierNotesCtrl.text.trim().isEmpty
                    ? null
                    : orderSupplierNotesCtrl.text.trim(),
              }));
  ...
  Future<void> _updateOrderStatus(int oid, String status,
      {String? buyerNote, String? supplierNote}) async {
    ...
    try {
      ...
      final r = await http.patch(
          Uri.parse('${widget.baseUrl}/agriculture/orders/$oid'),
          headers: await _agriHeaders(json: true),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: img.isEmpty
              ? const Center(
                  child:
                      Icon(Icons.eco_outlined, size: 40, color: Colors.white70))
              : null,
        ),
        ...
        Text(
            '$price SYP${unit.isNotEmpty ? ' /$unit' : ''} · MOQ ${minQty ?? '-'} ${unit.isNotEmpty ? unit : ''}',
        ...
        Row(
          children: [
            ...
            if ((p['availability_to'] ?? '').toString().isNotEmpty)
              Chip(
                  label: Text(
                      'Until ${(p['availability_to'] ?? '').toString().split('T').first}')),
          ],
        ),
        if (lead != null)
          Text('Lead time: $lead days',
              style: Theme.of(context).textTheme.bodySmall),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          children: [
            OutlinedButton(
                onPressed: () => _createOrderFromListing(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                      '$price SYP${unit.isNotEmpty ? ' /$unit' : ''} · MOQ $minQty'),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  TextField(
                      controller: rfqProductCtrl,
                      decoration: const InputDecoration(labelText: 'Product')),
                  TextField(
                      controller: rfqQtyCtrl,
                      keyboardType: TextInputType.number,
                      decoration: const InputDecoration(labelText: 'Quantity')),
                  TextField(
                      controller: rfqUnitCtrl,
                      decoration: const InputDecoration(
                          labelText: 'Unit (kg/ton/etc)')),
                  TextField(
                      controller: rfqCityCtrl,
                      decoration: const InputDecoration(labelText: 'City')),
                  TextField(
                      controller: rfqDateCtrl,
                      decoration: const InputDecoration(
                          labelText: 'Target date (ISO)')),
                  TextField(
                      controller: rfqBuyerCtrl,
                      decoration: const InputDecoration(
                          labelText: 'Buyer wallet (optional)')),
                  TextField(
                      controller: rfqNotesCtrl,
        ...
        GlassPanel(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    SizedBox(
                        ...
                        child: TextField(
                          controller: cityCtrl,
                          decoration:
                              const InputDecoration(labelText: 'City/Region'),
                        )),
                    SizedBox(
                        width: 160,
                        child: TextField(
                ...
                if (listingsOut.isNotEmpty)
                  Padding(
                      padding: const EdgeInsets.only(top: 6),
                      child: Text(listingsOut)),
              ],
            )),
        const SizedBox(height: 12),
        Wrap(
          ...
          runSpacing: 12,
          children: listings
              .map((p) => SizedBox(width: 320, child: _listingCard(p, l)))
              .toList(),
        ),
      ],
    );
              ...
              children: [
                ...
                SizedBox(
                    ...
                    child: TextField(
                        controller: lVarCtrl,
                        decoration:
                            const InputDecoration(labelText: 'Variety'))),
                SizedBox(
                    width: 140,
                    child: TextField(
                ...
                SizedBox(
                    width: 140,
                    child: TextField(
                        controller: lOriginCtrl,
                        decoration:
                            const InputDecoration(labelText: 'Origin'))),
                SizedBox(
                    width: 180,
                    child: TextField(
                        controller: lCertCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Certifications'))),
                SizedBox(
                    width: 140,
                    child: TextField(
                        controller: lStatusCtrl,
                        decoration:
                            const InputDecoration(labelText: 'Status'))),
                SizedBox(
                    width: 200,
                    child: TextField(
                        controller: lSellerCtrl,
                        decoration:
                            const InputDecoration(labelText: 'Seller wallet'))),
                ElevatedButton(
                    onPressed: _createListing,
                    child: const Text('Save listing')),
              ],
            ),
            if (listingOut.isNotEmpty)
              Padding(
                  padding: const EdgeInsets.only(top: 6),
                  child: Text(listingOut)),
          ]),
        ),
        const SizedBox(height: 12),
        GlassPanel(
              ...
              children: [
                SizedBox(
                    ...
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'Listing ID'))),
                SizedBox(
                    width: 140,
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'New price (cents)'))),
                SizedBox(
                    width: 140,
                    child: TextField(
                        controller: opListingStatusCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Status (listed/paused/out_of_stock)'))),
                ElevatedButton(
                    onPressed: () {
                      final id = int.tryParse(opListingIdCtrl.text.trim()) ?? 0;
                      if (id > 0 && opListingPriceCtrl.text.trim().isNotEmpty) {
                        _updateListingPrice(id,
                            int.tryParse(opListingPriceCtrl.text.trim()) ?? 0);
                      }
                    },
                ...
                OutlinedButton(
                    onPressed: () {
                      final id = int.tryParse(opListingIdCtrl.text.trim()) ?? 0;
                      if (id > 0 &&
                          opListingStatusCtrl.text.trim().isNotEmpty) {
                        _updateListingStatus(
                            id, opListingStatusCtrl.text.trim());
                      }
                    },
                          ...
                          Wrap(
                            ...
                            children: [
                              SizedBox(
                                  width: 120,
                                  child: TextField(
                                      controller: replyPriceCtrl,
                                      keyboardType: TextInputType.number,
                                      decoration: const InputDecoration(
                                          labelText: 'Price/unit (cents)'))),
                              SizedBox(
                                  width: 120,
                                  child: TextField(
                                      controller: replyEtaCtrl,
                                      keyboardType: TextInputType.number,
                                      decoration: const InputDecoration(
                                          labelText: 'ETA days'))),
                              SizedBox(
                              ...
                              SizedBox(
                                  width: 200,
                                  child: TextField(
                                      controller: replyWalletCtrl,
                                      decoration: const InputDecoration(
                                          labelText: 'Supplier wallet'))),
                              ElevatedButton(
                                  onPressed: () => _replyRFQ(rid),
                                  child: const Text('Reply')),
                              OutlinedButton(
                                  onPressed: () => _loadReplies(rid),
                              ...
                              child: Column(
                                ...
                                children: replies.map((rep) {
                                  ...
                                  return Padding(
                                    ...
                                    child: Text(
                                        'Reply: $pp cents · ETA $eta d · ${msg.isEmpty ? '' : msg}'),
                                  );
                                }).toList(),
                              ),
                            ),
            ...
            child:
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton(
                      onPressed: () =>
                          setState(() => showOrderForm = !showOrderForm),
                      child: Text(showOrderForm
                          ? 'Hide order form'
                          : 'Quick create order')),
                ],
              ),
              ...
              SizedBox(
                  ...
                  child: TextField(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Status filter (pending/...)'),
                  ...
                  children: [
                    ...
                    SizedBox(
                        ...
                        child: TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Buyer wallet'))),
                    SizedBox(
                        width: 160,
                        child: TextField(
                            controller: orderSellerCtrl,
                            decoration: const InputDecoration(
                                labelText: 'Seller wallet'))),
                    SizedBox(
                        width: 200,
                        child: TextField(
                            controller: orderBuyerNotesCtrl,
                            decoration: const InputDecoration(
                                labelText: 'Buyer notes'))),
                    SizedBox(
                        width: 200,
                        child: TextField(
                            controller: orderSupplierNotesCtrl,
                            decoration: const InputDecoration(
                                labelText: 'Supplier notes'))),
                    ElevatedButton(
                        onPressed: () {
                  ...
                  child: GlassPanel(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        TextField(
                          decoration:
                              const InputDecoration(labelText: 'Buyer notes'),
                          controller: TextEditingController(text: buyerNote),
                          onSubmitted: (val) => _updateOrderStatus(
                              id is int ? id : int.tryParse(id.toString()) ?? 0,
                              status,
                              buyerNote: val),
                        ),
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            if (!kEnduserOnly) Tab(text: l.isArabic ? 'مشغل' : 'Operator'),
          ],
        ),
      ),
      body: Stack(
