...
class _RealEstateEnduserState extends State<RealEstateEnduser> {
  ...
  String _sort =
      'newest'; // newest | price_desc | price_asc | area_desc | area_asc
  Set<int> _savedIds = {};
  ...
  Future<void> _loadSaved() async {
    try {
      ...
      _savedIds =
          raw.map((e) => int.tryParse(e) ?? -1).where((e) => e > 0).toSet();
      if (mounted) setState(() {});
    } catch (_) {}
  }
  ...
  Future<void> _saveSaved() async {
    try {
      ...
      await sp.setStringList(
          're_saved_ids', _savedIds.map((e) => e.toString()).toList());
    } catch (_) {}
  }
  ...
  List<Map<String, dynamic>> _applySortFilter(List<Map<String, dynamic>> list) {
    ...
    if (_statusFilter.isNotEmpty) {
      out = out
          .where((e) =>
              (e['status'] ?? '').toString().toLowerCase() == _statusFilter)
          .toList();
    }
    out = out.where((e) {
      final baths = (e['bathrooms'] is num)
          ? (e['bathrooms'] as num).toDouble()
          : double.tryParse((e['bathrooms'] ?? '').toString()) ?? 0;
      final area = (e['area_sqm'] is num)
          ? (e['area_sqm'] as num).toDouble()
          : double.tryParse((e['area_sqm'] ?? '').toString()) ?? 0;
      if (minBaths > 0 && baths < minBaths) return false;
      if (minArea > 0 && area < minArea) return false;
      return true;
    }).toList();
    int priceOf(Map<String, dynamic> m) => (m['price_cents'] is int)
        ? m['price_cents'] as int
        : int.tryParse((m['price_cents'] ?? '').toString()) ?? 0;
    int idOf(Map<String, dynamic> m) => (m['id'] is int)
        ? m['id'] as int
        : int.tryParse((m['id'] ?? '').toString()) ?? 0;
    double areaOf(Map<String, dynamic> m) => (m['area_sqm'] is num)
        ? (m['area_sqm'] as num).toDouble()
        : double.tryParse((m['area_sqm'] ?? '').toString()) ?? 0.0;
    switch (_sort) {
      case 'price_desc':
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (r.statusCode == 200) {
        ...
        if (body is List) {
          _props = _applySortFilter(body
              .map<Map<String, dynamic>>((e) => (e as Map).cast())
              .toList());
  ...
  Future<void> _inquiry(int pid) async {
    ...
    try {
      ...
      final r =
          await http.post(Uri.parse('${widget.baseUrl}/realestate/inquiries'),
              headers: h,
              body: jsonEncode({
                'property_id': pid,
                'name': inNameCtrl.text.trim().isEmpty
                    ? 'Guest'
                    : inNameCtrl.text.trim(),
                'phone': inPhoneCtrl.text.trim().isEmpty
                    ? null
                    : inPhoneCtrl.text.trim(),
                'message': inMsgCtrl.text.trim().isEmpty
                    ? null
                    : inMsgCtrl.text.trim(),
              }));
  ...
  Future<void> _reserve(int pid) async {
    ...
    try {
      ...
      final r =
          await http.post(Uri.parse('${widget.baseUrl}/realestate/reserve'),
              headers: h,
              body: jsonEncode({
                'property_id': pid,
                'buyer_wallet_id': reserveWalletCtrl.text.trim(),
                'deposit_cents':
                    int.tryParse(reserveDepositCtrl.text.trim()) ?? 0
              }));
      setState(() => _actionOut = '${r.statusCode}: ${r.body}');
    } catch (e) {
      setState(() => _actionOut = 'error: $e');
                      ...
                      Row(
                        children: [
                          ...
                          IconButton(
                              ...
                              icon: Icon(_savedIds.contains(pid)
                                  ? Icons.bookmark
  ...
  Widget build(BuildContext context) {
        ...
        child: Center(
          child: Row(
            ...
            children: [
              ...
              Text(
                l.isArabic ? 'الخريطة غير متاحة حالياً' : 'Map preview unavailable',
    ...
    Widget cards = Column(
      children: _props.map((p) {
        ...
        final areaNum = (area is num)
            ? area.toDouble()
            : double.tryParse(area?.toString() ?? '');
        final pricePerSqm =
            (areaNum != null && areaNum > 0) ? (price / areaNum).round() : null;
        final priceStr =
            _fmtNum(price is num ? price : int.tryParse(price.toString()) ?? 0);
                ...
                Wrap(
                  ...
                  children: [
                    ElevatedButton(
                        ...
                        child:
                            Text(l.isArabic ? 'عرض التفاصيل' : 'View details')),
                    OutlinedButton(
                        onPressed: () {
                          _setFormPid(pid);
                          _inquiry(pid);
            ...
            children: [
              ...
              SizedBox(
                  ...
                  child: TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'Buyer wallet id'),
                  )),
              SizedBox(
                  width: 160,
                  child: TextField(
                ...
                Wrap(
                  ...
                  children: _recentIds.map((pid) {
                    return ActionChip(
                        label: Text('ID $pid'),
                        onPressed: () {
                          final match = _props.firstWhere((e) {
                            final id = e['id'] ?? '';
                            final eid = (id is int)
                                ? id
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await http.get(
          Uri.parse('${widget.baseUrl}/realestate/properties?limit=200'),
  ...
  Future<void> _create() async {
    ...
    try {
      final body = {
        ...
        'bedrooms': bedsCtrl.text.trim().isEmpty
            ? null
            : int.tryParse(bedsCtrl.text.trim()),
        'bathrooms': bathsCtrl.text.trim().isEmpty
            ? null
            : int.tryParse(bathsCtrl.text.trim()),
        'area_sqm': areaCtrl.text.trim().isEmpty
            ? null
      ...
      final r = await http.post(
          Uri.parse('${widget.baseUrl}/realestate/properties'),
                          ...
                          children: [
                            DropdownButton<String>(
                              ...
                              items: const [
                                ...
                                DropdownMenuItem(
                                    value: 'rented', child: Text('Rented')),
                              ],
                              onChanged: (v) {
                                setState(() {
                                  _statusFilter = v ?? 'all';
                            ...
                            SizedBox(
                                width: 180,
                                child: TextField(
                                    controller: _opQueryCtrl,
                                    decoration: const InputDecoration(
                                        labelText: 'Search title'))),
                            SizedBox(
                                width: 160,
                                child: TextField(
                                    controller: _opCityCtrl,
                                    decoration: const InputDecoration(
                                ...
                                children: [
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      OutlinedButton(
                                          onPressed: () =>
                                              _updateStatus(pid, 'listed'),
                                          child: const Text('Listed')),
                                      OutlinedButton(
                                          onPressed: () =>
                                              _updateStatus(pid, 'reserved'),
                                          child: const Text('Reserved')),
                                      OutlinedButton(
                                          onPressed: () =>
                                              _updateStatus(pid, 'sold'),
                                          child: const Text('Sold')),
                                      OutlinedButton(
                                          onPressed: () =>
                                              _updateStatus(pid, 'rented'),
      ...
      child: DataTable(
        ...
        rows: props.map((p) {
          ...
          return DataRow(cells: [
            ...
            DataCell(Text(title)),
            DataCell(Text(city)),
            DataCell(Text(_fmtNumLocal(
                price is num ? price : int.tryParse(price.toString()) ?? 0))),
