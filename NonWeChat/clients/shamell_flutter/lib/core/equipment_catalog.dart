...
class _SavedAddress {
  ...
  const _SavedAddress(
      {required this.label, required this.address, this.lat, this.lon});
  ...
  Map<String, dynamic> toJson() =>
      {'label': label, 'address': address, 'lat': lat, 'lon': lon};
}
...
class _EquipmentCatalogPageState extends State<EquipmentCatalogPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (subcatCtrl.text.isNotEmpty)
        params['subcategory'] = subcatCtrl.text.trim();
      if (radiusCtrl.text.isNotEmpty)
      ...
      final uri = Uri.parse('${widget.baseUrl}/equipment/assets')
      ...
      child: Center(
        child: WaterButton(
          ...
          onTap: () => Navigator.of(context).push(MaterialPageRoute(
              builder: (_) =>
                  EquipmentOpsDashboardPage(baseUrl: widget.baseUrl))),
        ),
      ),
    );
  }
            ...
            children: [
              ...
              SizedBox(
                ...
                child: DropdownButtonFormField<String?>(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Unterkategorie'),
                  items: [
                    const DropdownMenuItem<String?>(
                        value: null, child: Text('Alle')),
                    ...(_assortment != null
          ...
          child: Glass(
            ...
            child: Column(
              ...
              children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        image: m['image_url'] != null
                            ? DecorationImage(
                                image: NetworkImage(m['image_url']),
                                fit: BoxFit.cover)
                            : null,
                      ),
                    ),
                ...
                Text('Tag: $dayPrice ${m['currency'] ?? ''}',
                    style: const TextStyle(fontWeight: FontWeight.w600)),
                Text('Woche: $weekPrice ${m['currency'] ?? ''}',
                    style:
                        const TextStyle(fontSize: 11, color: Colors.white70)),
                Text('Monat: $monthPrice ${m['currency'] ?? ''}',
                    style:
                        const TextStyle(fontSize: 11, color: Colors.white70)),
                const Spacer(),
                Row(
                  ...
                  children: [
                    Text(
                        '${m['available_quantity'] ?? m['quantity'] ?? ''} verfügbar',
  ...
  Widget _mapList() {
    ...
    return Padding(
      padding: const EdgeInsets.all(12.0),
      child:
          ShamellMapWidget(center: center, initialZoom: 11, markers: markers),
    );
  }
}
...
class EquipmentDetailPage extends StatefulWidget {
  ...
  const EquipmentDetailPage(
      {super.key, required this.baseUrl, required this.asset, this.walletId});
  ...
  Future<void> _calcDelivery() async {
    try {
      final uri = Uri.parse('${widget.baseUrl}/osm/geocode')
          .replace(queryParameters: {'q': deliveryCtrl.text.trim()});
      ...
      if (originLat != null &&
          ...
          _destLat != null &&
          _destLon != null) {
        final km = _kmBetween(originLat, originLon, _destLat!, _destLon!)
            .toStringAsFixed(1);
      ...
      final payload = {
        ...
        'from_iso':
            DateFormat("yyyy-MM-ddTHH:mm:ss'Z'").format(DateTime.now().toUtc()),
        'to_iso': DateFormat("yyyy-MM-ddTHH:mm:ss'Z'")
            .format(DateTime.now().add(const Duration(days: 1)).toUtc()),
        'quantity': 1,
      ...
      body: AppBG(
        child: ListView(
          ...
          children: [
                        ...
                        .map((u) => Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                image: DecorationImage(
                                    image: NetworkImage(u), fit: BoxFit.cover),
                              ),
                            ))
                        .toList()
                    : [
                        Container(
                          decoration: BoxDecoration(
                              ...
                              color: Colors.black12),
                          child: const Center(
                              child: Icon(Icons.construction, size: 48)),
                        )
                      ],
              ),
            ),
            ...
            Text(
                'Tag: ${asset['daily_rate_cents'] ?? '-'} ${asset['currency'] ?? ''}'),
            Text(
                'Woche: ${asset['weekly_rate_cents'] ?? '-'} ${asset['currency'] ?? ''}'),
            Text(
                'Monat: ${asset['monthly_rate_cents'] ?? '-'} ${asset['currency'] ?? ''}'),
            const SizedBox(height: 12),
            const Text('Add-ons',
                style: TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            const Text('• Schadenverzicht / Versicherung',
                style: TextStyle(fontSize: 12)),
            const Text('• Zubehör / Attachments auf Anfrage',
                style: TextStyle(fontSize: 12)),
            const SizedBox(height: 12),
            const Text('Lieferkosten kalkulieren',
                style: TextStyle(fontWeight: FontWeight.w700)),
            TextField(
                controller: deliveryCtrl,
                decoration: const InputDecoration(labelText: 'Lieferadresse')),
            const SizedBox(height: 6),
            Row(
              children: [
            ...
            WaterButton(
                ...
                onTap: () => Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) => EquipmentBookingWizard(
                        baseUrl: widget.baseUrl,
                        asset: asset,
                        walletId: widget.walletId)))),
          ],
...
class _EquipmentBookingWizardState extends State<EquipmentBookingWizard> {
  ...
  final List<String> _attachments = const [
    'Hydraulikhammer',
    'Schaufel',
    'Gabel',
    'Besen'
  ...
  Future<void> _quoteNow() async {
    ...
    try {
      ...
      if (originLat != null &&
          ...
          _deliveryLon != null) {
        deliveryKm =
            _kmBetween(originLat, originLon, _deliveryLat!, _deliveryLon!);
      }
      final body = {
  ...
  Future<void> _bookNow() async {
    ...
    try {
      ...
      final body = {
        ...
        'pickup_address':
            pickupCtrl.text.trim().isEmpty ? null : pickupCtrl.text.trim(),
        'confirm': true,
        'project':
            projectCtrl.text.trim().isEmpty ? null : projectCtrl.text.trim(),
        'po': poCtrl.text.trim().isEmpty ? null : poCtrl.text.trim(),
        'insurance': _insurance,
        'damage_waiver': _damageWaiver,
        'delivery_scheduled_iso': windowStartCtrl.text.trim().isEmpty
            ? null
            : windowStartCtrl.text.trim(),
        'pickup_scheduled_iso': windowEndCtrl.text.trim().isEmpty
            ? null
            : windowEndCtrl.text.trim(),
        'delivery_window_start_iso': windowStartCtrl.text.trim().isEmpty
            ? null
            : windowStartCtrl.text.trim(),
        'delivery_window_end_iso': windowEndCtrl.text.trim().isEmpty
            ? null
            : windowEndCtrl.text.trim(),
  ...
  Future<void> _saveAddress(_SavedAddress addr) async {
    ...
    await sp.setStringList('equipment_saved_addresses_v2',
  ...
  Future<void> _geocodeAddress(String addr) async {
    ...
    try {
      final uri = Uri.parse('${widget.baseUrl}/osm/geocode')
          .replace(queryParameters: {'q': addr});
      final r = await http.get(uri);
      if (r.statusCode == 200) {
        final body = jsonDecode(r.body);
        if (body is List && body.isNotEmpty) {
        ...
        Row(
          children: [
            Expanded(
                child: TextField(
                    controller: fromCtrl,
                    decoration: const InputDecoration(labelText: 'Von (ISO)'))),
            const SizedBox(width: 8),
            Expanded(
                child: TextField(
                    controller: toCtrl,
                    decoration: const InputDecoration(labelText: 'Bis (ISO)'))),
            IconButton(
                onPressed: _pickDates, icon: const Icon(Icons.date_range)),
          ],
        ),
        Row(
          children: [
  ...
  Widget _step2() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('2. Adresse & Kontakt',
            style: TextStyle(fontWeight: FontWeight.w700)),
        TextField(
            controller: addressCtrl,
            decoration:
                const InputDecoration(labelText: 'Adresse / Baustelle')),
        const SizedBox(height: 6),
        Row(
          children: [
            WaterButton(
            ...
            WaterButton(
                ...
                onTap: () => _saveAddress(_SavedAddress(
                    label: projectCtrl.text.trim().isNotEmpty
        ...
        if (_deliveryLat != null && _deliveryLon != null)
          Padding(
            ...
            child: Text(
                'Pin: ${_deliveryLat?.toStringAsFixed(4)}, ${_deliveryLon?.toStringAsFixed(4)}',
                style: const TextStyle(fontSize: 12, color: Colors.white70)),
          ),
          ...
          Wrap(
            ...
            children: _addressBook
                .map((a) => ActionChip(
                      ...
                      label: Text(a.label.isNotEmpty ? a.label : a.address,
                          style: const TextStyle(fontSize: 12)),
                      onPressed: () => setState(() {
        ...
        TextField(
            ...
            decoration:
                const InputDecoration(labelText: 'Abholadresse (optional)')),
        TextField(
            controller: contactCtrl,
            decoration: const InputDecoration(labelText: 'Kontakt / Firma')),
        TextField(
            controller: notesCtrl,
            decoration: const InputDecoration(labelText: 'Hinweise')),
        TextField(
            controller: projectCtrl,
            decoration: const InputDecoration(labelText: 'Projekt / Kunde')),
        TextField(
            controller: poCtrl,
            decoration: const InputDecoration(labelText: 'PO / Referenz')),
        TextField(
            controller: windowStartCtrl,
            decoration: const InputDecoration(
                labelText: 'Lieferzeitfenster Start (ISO)')),
        TextField(
            controller: windowEndCtrl,
            decoration: const InputDecoration(
                labelText: 'Lieferzeitfenster Ende (ISO)')),
        CheckboxListTile(
          value: _insurance,
        ...
        const SizedBox(height: 8),
        const Text('Attachments / Zubehör',
            style: TextStyle(fontWeight: FontWeight.w700)),
        Wrap(
          spacing: 6,
        ...
        Row(
          children: [
            WaterButton(
                label: 'Zurück', onTap: () => setState(() => _step = 0)),
            const SizedBox(width: 12),
            WaterButton(
                label: 'Weiter',
                onTap: () async {
    ...
    return Column(
      ...
      children: [
        ...
        if (_quote != null) ...[
          ...
          Text(
              'Gesamt: ${_quote?['total_cents']} ${_quote?['currency'] ?? ''}'),
          if (projectCtrl.text.trim().isNotEmpty)
            Text('Projekt: ${projectCtrl.text}'),
          ...
          if (_selectedAttachments.isNotEmpty)
            Text('Attachments: ${_selectedAttachments.join(", ")}',
                style: const TextStyle(fontSize: 12)),
          if (_deliveryLat != null && _deliveryLon != null)
            Text(
                'Liefer-Pin: ${_deliveryLat?.toStringAsFixed(4)}, ${_deliveryLon?.toStringAsFixed(4)}',
                style: const TextStyle(fontSize: 12)),
        ],
        const SizedBox(height: 8),
        Row(
          children: [
            WaterButton(
                label: 'Zurück', onTap: () => setState(() => _step = 1)),
            const SizedBox(width: 12),
