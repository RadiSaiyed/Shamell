...
class CommerceOperatorPage extends StatefulWidget {
  final String defaultBaseUrl;
  const CommerceOperatorPage({
    super.key,
  ...
  @override
  State<CommerceOperatorPage> createState() => _CommerceOperatorPageState();
}

class _CommerceOperatorPageState extends State<CommerceOperatorPage> {
  ...
  Future<void> _health() async {
    ...
    try {
      final r = await http.get(
        Uri.parse('$_baseUrl/commerce/health'),
  ...
  Future<void> _loadProducts() async {
    setState(() => _listOut = 'Loading products...');
    try {
      final uri = Uri.parse('$_baseUrl/commerce/products_cached');
      ...
      if (r.statusCode == 200) {
        ...
        try {
          ...
          if (decoded is List) {
            setState(() => _listOut = 'Products: ${decoded.length}\n$body');
          } else {
            setState(() => _listOut = 'Products loaded\n$body');
          }
        } catch (_) {
          setState(() => _listOut = 'Products loaded\n$body');
        }
            ...
            Row(
              children: [
                ...
                OutlinedButton(
                  ...
                  child: const Text('Reload products (cached)'),
                ),
              ],
            ),
