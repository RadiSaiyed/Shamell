...
def test_e2e_login_and_mobility_history(client, monkeypatch):
    ...
    # Stub Bus list in the BFF so it always returns exactly one entry for the logged-in user.
    ...
    resp = client.get(
        "/me/mobility_history?status=completed&limit=10",
