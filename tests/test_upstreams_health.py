...
def test_upstreams_health_returns_mapping_even_without_base_urls(client, monkeypatch):
    ...
    # Food is handled internally and should report OK even without BASE_URL.
    for service in [
        "payments",
        "taxi",
        "bus",
    ...
    assert food.get("status_code") == 200
    assert (food.get("body") or {}).get("internal") is True
...
def test_upstreams_health_includes_status_code_and_body_when_base_url_set(
    ...
    # Food remains internal-only and should report OK without a BASE_URL.
    food = data.get("food") or {}
    assert (food.get("body") or {}).get("internal") is True
    assert food.get("mode") == "internal"
