...
def test_admin_stats_requires_admin(client, user_auth, admin_auth):
  ...
  bff._AUDIT_EVENTS.extend(  # type: ignore[attr-defined]
      [
          {"action": "pay_amount_guardrail", "from_wallet_id": "w1"},
          {"action": "pay_velocity_guardrail_wallet", "from_wallet_id": "w1"},
          {"action": "pay_velocity_guardrail_wallet", "from_wallet_id": "w2"},
      ]
  )
  ...
  # Expect 1 amount guardrail and 2 velocity guardrails
  assert guardrails.get("pay_amount_guardrail") == 1
  assert guardrails.get("pay_velocity_guardrail_wallet") == 2
