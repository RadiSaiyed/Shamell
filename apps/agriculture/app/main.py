...
def create_listing(req: ListingCreate, s: Session = Depends(get_session)):
    l = Listing(title=req.title.strip(), category=(req.category or None), price_cents=req.price_cents,
                city=(req.city or None), seller_wallet_id=(req.seller_wallet_id or None))
    s.add(l); s.commit(); s.refresh(l)
    return l


@router.get("/listings", response_model=List[ListingOut])
def list_listings(q: str = "", city: str = "", category: str = "", limit: int = 50, s: Session = Depends(get_session)):
    stmt = select(Listing)
    if q:
