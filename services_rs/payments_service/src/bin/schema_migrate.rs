use shamell_payments_service::{config::Config, db};
...
async fn main() {
    ...
    if let Err(e) = db::apply_versioned_schema_migrations(&pool, &cfg.db_schema).await {
        tracing::error!(error = %e, "payments versioned SQL migration failed");
        std::process::exit(2);
    }

    if let Err(e) = db::ensure_schema(&pool, &cfg.db_schema).await {
        tracing::error!(error = %e, "payments legacy schema convergence failed");
        std::process::exit(2);
    }

    tracing::info!(
        db_schema = ?cfg.db_schema,
        "payments schema migration completed"
    );
}
