...
Compatibility note:
- `PAYMENTS_INTERNAL_SECRET` / `CHAT_INTERNAL_SECRET` are now optional compatibility-only hop secrets once signed internal identity is enabled.
- `SECURITY_ALERT_WEBHOOK_INTERNAL_SECRET` is now optional compatibility-only for `/internal/security/alerts` once signed alert identity is enabled.

## Internal Service Identity (BFF -> Chat / Payments)

Chat and Payments now support caller-bound, audience-bound Ed25519-signed internal requests.
...
Operational notes:
- public key format: `caller=base64_ed25519_public_key`
- BFF emits both legacy v1 and audience-bound v2 signatures during rollout
- receivers can still accept v1 only when `*_REQUIRE_INTERNAL_IDENTITY_V2=false`
- BFF signs caller id, target audience, method, path/query, timestamp, nonce, and SHA-256(body) in v2
- receivers enforce timestamp skew and reject nonce replays per instance
- receivers reject audience mismatches to stop cross-service request replay
- in `prod|staging`, `CHAT_REQUIRE_INTERNAL_IDENTITY_V2` and `PAYMENTS_REQUIRE_INTERNAL_IDENTITY_V2` must stay `true`
- in `prod|staging`, legacy secret fallback must remain disabled for Chat/Payments

## Security Alert Identity (Security Reporter -> BFF)

The internal BFF alert ingress supports the same signed request model.

Environment variables:
- `BFF_SECURITY_ALERT_INTERNAL_IDENTITY_PUBLIC_KEYS`
- `BFF_SECURITY_ALERT_INTERNAL_IDENTITY_MAX_SKEW_SECS`
- `BFF_SECURITY_ALERT_REQUIRE_INTERNAL_IDENTITY_V2`
- `BFF_SECURITY_ALERT_ALLOW_LEGACY_INTERNAL_SECRET_FALLBACK`
- `SECURITY_ALERT_WEBHOOK_SIGNING_SEED_B64`
- `SECURITY_ALERT_WEBHOOK_SERVICE_ID`

Operational notes:
- `security_events_report.sh` and `security_alert_webhook_drill.sh` sign alert webhook posts when `SECURITY_ALERT_WEBHOOK_SIGNING_SEED_B64` is set.
- the expected audience for `/internal/security/alerts` is `bff`
- in `prod|staging`, `BFF_SECURITY_ALERT_REQUIRE_INTERNAL_IDENTITY_V2` must stay `true`
- in `prod|staging`, legacy secret fallback must remain disabled for alert ingress
...
These endpoints can proxy paid routing or map APIs and should be rate-limited.
