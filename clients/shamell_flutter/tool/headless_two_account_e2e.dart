...
Future<AccountCtx> _createAccount(
  ...
  final username = 'e2e_${label.toLowerCase()}_${_randId(len: 12)}';
  ...
  if (signupResp.statusCode == 503) {
    throw E2eError('Username/password signup disabled on server (HTTP 503).');
  }
  _expect2xx(signupResp, 'auth/signup (${ctx.label})');
  ...
  if (ctx.shamellId.isEmpty) {
    throw E2eError('Missing shamell_id in signup response (${ctx.label})');
  }
  return ctx;
}
...
Future<String> _paymentsFavoriteCreate(
  ...
  final idem =
      'fav-create-${owner.label}-${favorite.label}-${DateTime.now().millisecondsSinceEpoch}-${_randId(len: 6)}';
  final r = await _req(
    client,
    method: 'POST',
...
Future<void> _paymentsFavoriteDelete(
  ...
  final idem =
      'fav-delete-${owner.label}-${DateTime.now().millisecondsSinceEpoch}-${_randId(len: 6)}';
  final r = await _req(
    client,
    method: 'DELETE',
    uri: Uri.parse(
...
Future<String> _paymentsRequestCreate(
  ...
  final idem =
      'req-create-${requester.label}-${payer.label}-${DateTime.now().millisecondsSinceEpoch}-${_randId(len: 6)}';
  final r = await _req(
    client,
    method: 'POST',
...
Future<void> _paymentsRequestCancel(
  ...
  final idem =
      'req-cancel-${requester.label}-${DateTime.now().millisecondsSinceEpoch}-${_randId(len: 6)}';
  final r = await _req(
    client,
    method: 'POST',
    uri: Uri.parse(
...
Future<CallSignalPair> _connectCallSignals({
  ...
  final wsHeadersA = <String, dynamic>{
    ..._chatHeaders(ctx: a),
    if ((_hostHeaderOverride ?? '').isNotEmpty) 'host': _hostHeaderOverride!,
  };
  final wsHeadersB = <String, dynamic>{
    ..._chatHeaders(ctx: b),
    if ((_hostHeaderOverride ?? '').isNotEmpty) 'host': _hostHeaderOverride!,
  };
...
Future<void> main(List<String> args) async {
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (a == '--base-url' && i + 1 < args.length) {
      baseUrl = args[++i];
    } else if (a == '--host-header' && i + 1 < args.length) {
      hostHeader = args[++i];
      ...
      stdout.writeln(
        ...
        '[--base-url <url>] [--host-header <host>] [--keep-sessions]',
      );
      return;
    } else {
  ...
  try {
    ...
    if (_hostHeaderOverride != null) {
      _printStep('Host header: $_hostHeaderOverride');
    }
    ...
    await _runCheck(failures, 'contacts invite/redeem', () async {
      final inviteToken =
