...
- `TRUSTED_TLS_CERTIFICATES_DER_BASE64` set to the semicolon-separated base64 DER certificate bundle used for release TLS pinning

Lanes (from `clients/shamell_flutter/ios`):
- `bundle exec fastlane build` → `flutter build ipa --release --obfuscate --split-debug-info=build/ios/symbols`
...
- `bundle exec fastlane build` → `flutter build appbundle --release --flavor user --obfuscate --split-debug-info=build/app/outputs/symbols/android`
