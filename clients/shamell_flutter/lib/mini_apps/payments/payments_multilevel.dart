...
class _PaymentsMultiLevelPageState extends State<PaymentsMultiLevelPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final r = await widget.api.getUri(
        uri,
        headers: await widget.api.sessionHeaders(),
      );
      if (r.statusCode != 200) {
        setState(() {
  ...
  void _openPayments() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PaymentsPage(widget.api.baseUrl, _walletId, did),
      ),
    );
  }
            ...
            Wrap(
              ...
              children: [
                for (final r in _roles
                    .where((r) => r == 'seller' || r.startsWith('operator_')))
                  Chip(
                    label: Text(r),
                    backgroundColor:
                ...
                if (!_roles
                    .any((r) => r == 'seller' || r.startsWith('operator_')))
                  Text(
                    l.isArabic
