...
class WeChatMomentsComposerPage extends StatefulWidget {
  ...
  @override
  State<WeChatMomentsComposerPage> createState() =>
      _WeChatMomentsComposerPageState();
}

class _WeChatComposerImage {
  final Uint8List bytes;
  final String mime;
  const _WeChatComposerImage({required this.bytes, required this.mime});
}

class _WeChatMomentsComposerPageState extends State<WeChatMomentsComposerPage> {
  ...
  Future<void> _openMentionPicker(int atIndex) async {
    ...
    final picked = await Navigator.of(context).push<_WeChatRemindPickResult>(
      MaterialPageRoute(
        builder: (_) => _WeChatRemindPickerPage(
  ...
  Future<void> _pickFromCamera() async {
    try {
      ...
      setState(() {
        if (_images.length >= _kMaxImages) return;
        _images.add(_WeChatComposerImage(
  ...
  Future<void> _pickFromAlbum() async {
    try {
      ...
      final loaded = <_WeChatComposerImage>[];
      for (final x in xs.take(remaining)) {
        try {
          final bytes = await x.readAsBytes();
          if (bytes.isEmpty) continue;
          loaded.add(_WeChatComposerImage(
              bytes: bytes, mime: _mimeFromFilename(x.name)));
        } catch (_) {}
      }
      if (!mounted) return;
  ...
  Future<void> _pickLocation() async {
    final picked = await Navigator.of(context).push<String?>(
      MaterialPageRoute(
        builder: (_) => _WeChatLocationPickerPage(
...
class _WeChatLocationEntry {
  final String label;
  final String? subtitle;
  const _WeChatLocationEntry({required this.label, this.subtitle});
}

class _WeChatLocationPickerPage extends StatefulWidget {
  final String? initialLabel;

  const _WeChatLocationPickerPage({this.initialLabel});

  @override
  State<_WeChatLocationPickerPage> createState() =>
      _WeChatLocationPickerPageState();
}

class _WeChatLocationPickerPageState extends State<_WeChatLocationPickerPage> {
  ...
  _WeChatLocationEntry? _current;

  bool _loadingSearch = false;
  String? _searchError;
  List<_WeChatLocationEntry> _results = const <_WeChatLocationEntry>[];
  ...
  _WeChatLocationEntry? _entryFromDisplay(String displayName) {
  ...
  Future<_WeChatLocationEntry?> _reverseGeocode({
    ...
    if (name.isNotEmpty) {
      ...
      return _WeChatLocationEntry(label: name);
    }
    final display = (decoded['display_name'] ?? '').toString().trim();
  ...
  Future<void> _search(String query) async {
    ...
    if (q.length < 2) {
      setState(() {
        ...
        _searchError = null;
        _results = const <_WeChatLocationEntry>[];
      });
      return;
    }
    ...
    try {
      ...
      final entries = <_WeChatLocationEntry>[];
...
enum _WeChatRemindSource {
  friends,
  contacts,
}

class _WeChatRemindPickResult {
  final List<String> ids;
  final List<String> names;
  const _WeChatRemindPickResult({
    required this.ids,
    required this.names,
  });
}

class _WeChatRemindHeader {
  final String letter;
  const _WeChatRemindHeader(this.letter);
}

class _WeChatRemindEntry {
  final String id;
  final String displayName;
  final String? subtitle;
  final String letter;
  const _WeChatRemindEntry({
    required this.id,
...
class _WeChatRemindPickerPage extends StatefulWidget {
  final String baseUrl;
  final bool singlePick;
  final List<String> initialSelectedIds;
  final List<String> initialSelectedNames;

  const _WeChatRemindPickerPage({
    required this.baseUrl,
  ...
  @override
  State<_WeChatRemindPickerPage> createState() =>
      _WeChatRemindPickerPageState();
}

class _WeChatRemindPickerPageState extends State<_WeChatRemindPickerPage> {
  final TextEditingController _searchCtrl = TextEditingController();
  final ScrollController _scrollCtrl = ScrollController();
  final Map<String, GlobalKey> _letterKeys = <String, GlobalKey>{};

  _WeChatRemindSource _source = _WeChatRemindSource.friends;
  bool _loadingFriends = true;
  String? _friendsError;
  List<_WeChatRemindEntry> _friends = const <_WeChatRemindEntry>[];

  bool _loadingContacts = false;
  bool _contactsPermissionDenied = false;
  String? _contactsError;
  List<_WeChatRemindEntry> _contacts = const <_WeChatRemindEntry>[];
  ...
  Future<void> _loadContacts() async {
    if (_loadingContacts || _contacts.isNotEmpty) return;
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final sourceEntries =
        _source == _WeChatRemindSource.friends ? _friends : _contacts;
    final loading = _source == _WeChatRemindSource.friends
        ? _loadingFriends
        : _loadingContacts;
    final error =
        _source == _WeChatRemindSource.friends ? _friendsError : _contactsError;
    final denied =
        _source == _WeChatRemindSource.contacts && _contactsPermissionDenied;
    ...
    for (final e in filtered) {
      if (currentLetter != e.letter) {
        ...
        items.add(_WeChatRemindHeader(currentLetter));
      }
      items.add(e);
    }
    ...
    for (final item in items) {
      if (item is _WeChatRemindHeader) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  isArabic
                      ? 'يلزم السماح بالوصول إلى جهات الاتصال.'
        ...
        children: [
          ListView.builder(
            ...
            itemBuilder: (ctx, i) {
              ...
              final e = item as _WeChatRemindEntry;
                ...
                TextButton(
                  onPressed: () {
                    ...
                    final names = ids
                        ...
                        .toList();
                    Navigator.of(context).pop(
                      _WeChatRemindPickResult(ids: ids, names: names),
                    );
                  },
                  child: Text(
                    doneLabel,
