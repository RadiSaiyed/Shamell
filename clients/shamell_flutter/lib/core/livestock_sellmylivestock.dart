...
class LivestockMarketplacePage extends StatefulWidget {
  ...
  @override
  State<LivestockMarketplacePage> createState() =>
      _LivestockMarketplacePageState();
}

class _LivestockMarketplacePageState extends State<LivestockMarketplacePage>
  ...
  void initState() {
    super.initState();
    _tab = TabController(length: kEnduserOnly ? 2 : 3, vsync: this);
  ...
  Future<void> _persistWatchlist() async {
    ...
    await sp.setStringList(
        'livestock_watchlist', _watchlist.map((e) => e.toString()).toList());
    final cacheJson =
        jsonEncode(_watchCache.map((k, v) => MapEntry(k.toString(), v)));
      ...
      if (r.statusCode != 200) {
        ...
        if (j is List) {
          ...
          setState(() {
            ...
            _avgPrice = _listings.isEmpty
                ? 0
                : (combinedPrice / _listings.length / 100);
          });
        }
      }
    } catch (e) {
  ...
  Future<void> _createListing() async {
    ...
    try {
      ...
      final r = await http.post(uri,
          headers: await _hdrLivestock(json: true), body: jsonEncode(body));
      if (r.statusCode >= 200 && r.statusCode < 300) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Create failed: ${r.statusCode} ${r.body}')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context)
  ...
  Future<void> _updateListing(int id, Map<String, dynamic> patch) async {
    try {
      ...
      final r = await http.patch(uri,
          headers: await _hdrLivestock(json: true), body: jsonEncode(patch));
      if (r.statusCode >= 200 && r.statusCode < 300) {
        await _loadListings();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Update failed: ${r.statusCode} ${r.body}')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context)
          ...
          Row(
            children: [
              Expanded(
                child: TextField(
                  decoration:
                      const InputDecoration(labelText: 'Min price (cents)'),
              ...
              Expanded(
                child: TextField(
                  decoration: const InputDecoration(
                      labelText: 'Max price (cents, optional)'),
                  keyboardType: TextInputType.number,
                  onChanged: (v) {
                    final n = double.tryParse(v) ?? 0;
                    setState(() {
              ...
              Expanded(
                child: TextField(
                  ...
                  keyboardType: TextInputType.number,
                  onChanged: (v) =>
                      setState(() => _minWeight = double.tryParse(v) ?? 0),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: TextField(
                  ...
                  keyboardType: TextInputType.number,
                  onChanged: (v) =>
                      setState(() => _maxWeight = double.tryParse(v) ?? 0),
                ),
              ),
              const SizedBox(width: 12),
              ElevatedButton.icon(
  ...
  Widget _buildCard(dynamic item) {
    ...
    final priceCents = (item['price_cents'] ?? 0) is num
        ? item['price_cents'] as num
        : num.tryParse(item['price_cents'].toString()) ?? 0;
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(
                            child: Text(item['title'] ?? 'Listing',
                                style: const TextStyle(
                                    fontWeight: FontWeight.w700))),
                        IconButton(
                          visualDensity: VisualDensity.compact,
                          onPressed: () => _toggleWatch(item['id'] as int,
                              item is Map<String, dynamic> ? item : null),
                          icon: Icon(
                            watched ? Icons.bookmark : Icons.bookmark_border,
                            color: watched
                                ? Tokens.colorAgricultureLivestock
                                : null,
                          ),
                    ...
                    Wrap(
                      ...
                      children: [
                        if (item['species'] != null)
                          _chip(item['species'].toString().toUpperCase(),
                              Tokens.colorAgricultureLivestock),
                        if (item['breed'] != null)
                          _chip(item['breed'], Colors.blueGrey),
                        _chip(
                        ...
                        if (grade != null)
                          _chip('Grade ${grade.toString()}', Colors.deepPurple),
                        if (health != null)
                          _chip(health.toString(), Colors.teal),
                      ],
                    ),
                    const SizedBox(height: 4),
                    Text(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          onPressed: () => _updateListing(
                              item['id'] as int, {'negotiable': !negotiable}),
                          icon: Icon(negotiable
                              ? Icons.lock_open
                              : Icons.lock_outline),
                          label: Text(
                              negotiable ? 'Mark fixed' : 'Mark negotiable'),
                        ),
                        if (tags.isNotEmpty)
                          Wrap(
                            spacing: 4,
            ...
            child: ListView(
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    if (item['species'] != null)
                      _chip(item['species'], Tokens.colorAgricultureLivestock),
                    if (item['breed'] != null)
                      _chip(item['breed'], Colors.blueGrey),
                    if (item['city'] != null)
                      _chip(item['city'], Colors.orange),
                    _chip(item['status'] ?? 'available', Tokens.colorPayments),
                    _chip((item['negotiable'] == true) ? 'Negotiable' : 'Fixed',
                        Colors.teal),
                    if (item['quality_grade'] != null)
                      _chip(
                          'Grade ${item['quality_grade']}', Colors.deepPurple),
                    if (item['health_status'] != null)
                      _chip(item['health_status'], Colors.teal),
                ...
                const SizedBox(height: 12),
                Text(
                    'Contact: ${(item['seller_phone'] ?? '—').toString()} • Farm: ${(item['farm_name'] ?? '—').toString()}'),
  ...
  Widget _buildMarketplace() {
    ...
    if (_species.isNotEmpty)
      active.add(_filterChip('Species: ${_species.toUpperCase()}', () {
        setState(() => _species = '');
        _loadListings();
      }));
    if (_status.isNotEmpty)
      active.add(_filterChip('Status: $_status', () {
        setState(() => _status = '');
        _loadListings();
      }));
    if (_sex.isNotEmpty)
      active.add(_filterChip('Sex: $_sex', () {
        setState(() => _sex = '');
        _loadListings();
      }));
    if (_city.isNotEmpty)
      active.add(_filterChip('City: $_city', () {
        setState(() => _city = '');
        _loadListings();
      }));
    if (_minPrice > 0 || (_hasMax && _maxPrice > 0)) {
      active.add(_filterChip(
          'Price ${_minPrice > 0 ? "≥ ${_minPrice.toInt()}" : ""} ${_hasMax && _maxPrice > 0 ? "≤ ${_maxPrice.toInt()}" : ""}',
          () {
        setState(() {
          _minPrice = 0;
          _maxPrice = 0;
    ...
    if (_minWeight > 0 || _maxWeight > 0) {
      active.add(_filterChip(
          'Weight ${_minWeight > 0 ? "≥ ${_minWeight.toInt()}kg" : ""} ${_maxWeight > 0 ? "≤ ${_maxWeight.toInt()}kg" : ""}',
          () {
        setState(() {
          _minWeight = 0;
          _maxWeight = 0;
          ...
          Wrap(
            ...
            children: [
              ...
              if (_speciesCounts.isNotEmpty)
                Chip(
                  label: Text(
                    'Top: ${(_speciesCounts.entries.toList()..sort((a, b) => b.value.compareTo(a.value))).first.key.toUpperCase()}',
                  ),
  ...
  Widget _buildSaved() {
    ...
    final fromFeed =
        _listings.where((e) => savedIds.contains(e['id'])).toList();
    final cached =
        savedIds.where((id) => !fromFeed.any((e) => e['id'] == id)).map((id) {
    ...
    if (savedListings.isEmpty) {
      return ListView(
        ...
        children: const [
          Text(
              'No saved livestock yet. Tap the bookmark on any listing to save it.'),
        ],
      );
    }
          ...
          Wrap(
            ...
            children: [
              ...
              ActionChip(
                ...
                label:
                    Text(_negotiableOnly ? 'Negotiable on' : 'Negotiable off'),
                onPressed: () {
                  setState(() {
        ...
        GlassPanel(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              TextField(
                  controller: _createTitle,
                  decoration: const InputDecoration(labelText: 'Title *')),
              TextField(
                  controller: _createPrice,
                  decoration: const InputDecoration(labelText: 'Price cents *'),
                  keyboardType: TextInputType.number),
              TextField(
                  controller: _createSpecies,
                  decoration: const InputDecoration(
                      labelText: 'Species (cattle/sheep/...)')),
              TextField(
                  controller: _createBreed,
                  decoration: const InputDecoration(labelText: 'Breed')),
              TextField(
                  controller: _createWeight,
                  decoration: const InputDecoration(labelText: 'Weight kg'),
                  keyboardType: TextInputType.number),
              TextField(
                  controller: _createCity,
                  decoration: const InputDecoration(labelText: 'City')),
              TextField(
                  controller: _createFarm,
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) =>
                    setState(() => _createStatus = v ?? 'available'),
                      ...
                      DropdownButton<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'available', child: Text('Available')),
                          DropdownMenuItem(
                              value: 'pending', child: Text('Reserved')),
                          DropdownMenuItem(value: 'sold', child: Text('Sold')),
                        ],
                        onChanged: (v) {
                          if (v == null) return;
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: TextField(
                          decoration: const InputDecoration(
                              labelText: 'Update price (cents)'),
                          keyboardType: TextInputType.number,
                          onSubmitted: (v) {
                            final n = int.tryParse(v.trim());
                            if (n != null)
                              _updateListing(
                                  item['id'] as int, {'price_cents': n});
                          },
                        ),
                      ),
  ...
  Widget build(BuildContext context) {
    ...
    final tabs = <Tab>[
      ...
      if (!kEnduserOnly) const Tab(text: 'Operator'),
      ...
      body: Stack(
        children: [
          ...
          TabBarView(
            ...
            children: [
              _buildMarketplace(),
              if (!kEnduserOnly) _buildOperator(),
              _buildSaved(),
            ],
          ),
        ],
...
class _OfferSheetState extends State<_OfferSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final uri = Uri.parse(
          '${widget.baseUrl}/livestock/listings/${widget.listingId}/offers');
      final r = await http.post(uri,
          headers: await _hdrLivestock(json: true), body: jsonEncode(body));
      if (r.statusCode >= 200 && r.statusCode < 300) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Offer failed: ${r.statusCode} ${r.body}')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Offer failed: $e')));
    } finally {
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
              controller: _price,
              decoration:
                  const InputDecoration(labelText: 'Offer price (cents)'),
              keyboardType: TextInputType.number),
          TextField(
              controller: _qty,
              decoration:
                  const InputDecoration(labelText: 'Quantity / headcount'),
              keyboardType: TextInputType.number),
          TextField(
              controller: _phone,
          ...
          TextField(
              ...
              decoration:
                  const InputDecoration(labelText: 'Preferred delivery date')),
          const SizedBox(height: 12),
          ElevatedButton.icon(
...
class _OfferListSheetState extends State<_OfferListSheet> {
  ...
  Future<void> _update(int id, String status) async {
    try {
      ...
      final r = await http.patch(uri,
          headers: await _hdrLivestock(json: true),
  ...
  Future<void> _copyOffersCsv(List<dynamic> items) async {
    ...
    final buf = StringBuffer();
    buf.writeln(
        'id,buyer_name,buyer_phone,offer_price_cents,currency,quantity,status,delivery_city,preferred_date,note');
  ...
  Widget build(BuildContext context) {
    ...
    final avgValue =
        visibleList.isEmpty ? 0 : (totalValue / visibleList.length);
    return Padding(
      padding: const EdgeInsets.all(16),
      child: Column(
        mainAxisSize: MainAxisSize.min,
          ...
          if (_offers.isNotEmpty) ...[
            ...
            Wrap(
              ...
              children: [
                Chip(
                  label:
                      Text('Offers: ${visibleList.length}/${_offers.length}'),
                  avatar: const Icon(Icons.list_alt, size: 18),
                ),
                Chip(
                ...
                if (visibleList.isNotEmpty)
                  Chip(
                    label:
                        Text('Lowest: ${(minValue / 100).toStringAsFixed(2)}'),
                ...
                ActionChip(
                  ...
                  label: Text(_statusFilter.isEmpty
                      ? 'All statuses'
            ...
            ...visibleList.map((o) => ListTile(
                  title: Row(
                    children: [
                      Expanded(
                          child: Text(
                              '${o['buyer_name'] ?? 'Buyer'} • ${o['buyer_phone'] ?? ''}')),
                      Chip(
                        label: Text((o['status'] ?? 'open').toString()),
                        backgroundColor:
                            _statusColor((o['status'] ?? '').toString()),
                      ),
                    ],
                  ),
                  subtitle: Column(
                    ...
                    children: [
                      Text(
                          'Offer: ${(o['offer_price_cents'] ?? 0) / 100} ${o['currency'] ?? ''} • qty: ${o['quantity'] ?? '—'}'),
  ...
  Color _statusColor(String status) {
    switch (status.toLowerCase()) {
      case 'accepted':
        return Tokens.colorPayments.withValues(alpha: 0.15);
      case 'declined':
