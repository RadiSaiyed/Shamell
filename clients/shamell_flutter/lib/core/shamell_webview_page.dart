...
const Set<String> _shamellWebViewAllowedExternalSchemes = <String>{
  'http',
  'https',
  'mailto',
  'tel',
...
const String _webViewSessionCookieName = '__Host-sa_session';
const String _webViewLegacySessionCookieName = 'sa_session';

@visibleForTesting
bool shamellWebViewIsLocalhostHost(String host) {
  final h = host.trim().toLowerCase();
  return h == 'localhost' || h == '127.0.0.1' || h == '::1';
}

@visibleForTesting
bool shamellWebViewIsSameOrigin(Uri a, Uri b) {
...
@visibleForTesting
bool shamellWebViewAllowsJavaScript({
  required Uri initialUri,
  Uri? baseUri,
}) {
  final scheme = initialUri.scheme.toLowerCase();
  if (scheme != 'http' && scheme != 'https') {
    return false;
  }
  if (initialUri.host.trim().isEmpty || initialUri.userInfo.isNotEmpty) {
    return false;
  }
  if (scheme == 'http' && !shamellWebViewIsLocalhostHost(initialUri.host)) {
    return false;
  }
  if (baseUri == null) {
    return false;
  ...
  if (baseUri.host.trim().isEmpty || baseUri.userInfo.isNotEmpty) {
    return false;
  }
  return shamellWebViewIsSameOrigin(initialUri, baseUri);
}

@visibleForTesting
Uri? normalizeShamellWebViewExternalUri(Uri uri) {
  final scheme = uri.scheme.toLowerCase();
  if (!_shamellWebViewAllowedExternalSchemes.contains(scheme)) {
    return null;
  }
  if (scheme == 'http' || scheme == 'https') {
    if (uri.host.trim().isEmpty) return null;
    if (uri.userInfo.isNotEmpty) return null;
    if (scheme == 'http' && !shamellWebViewIsLocalhostHost(uri.host)) {
      return null;
    }
    return uri;
  }
  if (uri.path.trim().isEmpty) return null;
  return uri;
}

@visibleForTesting
bool shamellWebViewCopiedLinkIsSensitive(Uri uri, {Uri? baseUri}) {
  final scheme = uri.scheme.toLowerCase();
  if (scheme != 'http' && scheme != 'https') {
    return false;
  }
  if (baseUri != null && shamellWebViewIsSameOrigin(uri, baseUri)) {
    return true;
  }
  return uri.hasQuery || uri.fragment.trim().isNotEmpty;
}

@visibleForTesting
List<WebViewCookie> buildShamellWebViewSessionBridgeCookies({
  required Uri baseUri,
  String? sessionToken,
}) {
  final cookies = <WebViewCookie>[
    // webview_flutter cannot express real __Host- cookie semantics because it
    // requires a domain and cannot mark the cookie HttpOnly/Secure. Clear any
    // stale fake host-prefixed cookie first so it cannot shadow the dev bridge.
    WebViewCookie(
      name: _webViewSessionCookieName,
      value: '',
      domain: baseUri.host,
      path: '/',
    ),
    WebViewCookie(
      name: _webViewLegacySessionCookieName,
      value: '',
      domain: baseUri.host,
      path: '/',
    ),
...
class _ShamellWebViewPageState extends State<ShamellWebViewPage> {
  ...
  bool _isLocalhostHost(String host) {
    return shamellWebViewIsLocalhostHost(host);
  }
          ...
          onNavigationRequest: (request) async {
            ...
            if (scheme == 'http' || scheme == 'https') {
              ...
              if (widget.restrictToBaseOrigin &&
                ...
                final externalUri = normalizeShamellWebViewExternalUri(uri);
                if (externalUri == null) {
                  _showBlockedSnack('Blocked unsafe external navigation.');
                  return NavigationDecision.prevent;
                }
                try {
  ...
  bool _isSameOrigin(Uri a, Uri b) {
    return shamellWebViewIsSameOrigin(a, b);
  }
  ...
  Future<void> _setWebViewSessionCookie({
    ...
    try {
      ...
      for (final cookie in buildShamellWebViewSessionBridgeCookies(
        baseUri: baseUri,
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    final url = current;
                    if (url == null) return;
                    final externalUri = normalizeShamellWebViewExternalUri(url);
                    if (externalUri == null) {
                      _showBlockedSnack(
                        isArabic
                            ? 'تم حظر فتح رابط غير آمن.'
                            : 'Blocked unsafe external link.',
                      );
                      return;
                    }
                    try {
