...
class _CourierLivePageState extends State<CourierLivePage> {
  ...
  String get _wsUrl => widget.baseUrl.replaceFirst(RegExp(r"^http"), "ws") + "/courier/ws";
  ...
  Future<void> _pushDriverEvent() async {
    ...
    final status = _statusCtl.text.trim().isEmpty ? "delivering" : _statusCtl.text.trim();
    ...
    try {
      ...
      final r = await http.post(Uri.parse('${widget.baseUrl}/courier/orders/$oid/status'),
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Row(
              children: [
                Expanded(child: TextField(controller: _orderIdCtl, decoration: const InputDecoration(labelText: 'Order id'))),
                const SizedBox(width: 8),
                SizedBox(width: 140, child: TextField(controller: _statusCtl, decoration: const InputDecoration(labelText: 'Status (pickup/delivering/delivered)'))),
                const SizedBox(width: 8),
                Expanded(child: TextField(controller: _photoUrlCtl, decoration: const InputDecoration(labelText: 'Photo URL (as note)'))),
                const SizedBox(width: 8),
                ElevatedButton(onPressed: _pushDriverEvent, child: const Text('Send driver update')),
              ],
            ),
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: _drivers.values
                        .map((d) => Chip(label: Text('D${d['id']} ${d['status']} @ ${d['lat']},${d['lng']}')))
                        .toList(),
                  ),
                  ...
                  Wrap(
                    spacing: 8,
                    children: _orders.map((o) => Chip(label: Text('${o['id']} • ${o['status']}'))).toList(),
                  ),
                  const SizedBox(height: 8),
                  const Text('Log'),
                  ..._log.reversed.take(20).map((l) => Text(l, style: Theme.of(context).textTheme.bodySmall)),
                ],
              ),
            ),
          ],
