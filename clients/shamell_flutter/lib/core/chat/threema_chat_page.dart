...
import '../wechat_settings_hub_page.dart';
...
import 'wechat_chat_info_page.dart';

enum _WeChatComposerPanel { none, stickers, more }

class _WeChatChatComposerDraft {
  final String text;
...
class ThreemaChatPage extends StatefulWidget {
  ...
  /// This is used when ThreemaChatPage is embedded inside the
  /// WeChat-style SuperApp shell, which already provides a
  /// bottom navigation bar for Chats/Contacts/Discover/Me.
  final bool showBottomNav;
  const ThreemaChatPage({
    super.key,
...
class _ThreemaChatPageState extends State<ThreemaChatPage> {
  ...
  final Map<String, _WeChatChatComposerDraft> _composerDraftByChatId =
      <String, _WeChatChatComposerDraft>{};
  Timer? _draftPersistTimer;
  bool _suppressDraftListener = false;
  bool _wechatVoiceMode = false;
  _WeChatComposerPanel _composerPanel = _WeChatComposerPanel.none;
  final PageController _wechatMorePanelCtrl = PageController();
  int _wechatMorePanelPage = 0;
  Future<_StickerPanelData>? _stickersPanelFuture;
  String _stickersActivePackId = 'classic_smileys';
  ...
  Map<String, String> _friendPhones = const <String, String>{};
  ...
  Future<void> _showContactInfo() async {
    ...
    final phone = (_friendPhones[peerId] ?? '').trim();
    final meName = () {
      final me = _me;
    ...
    await Navigator.push<void>(
      ...
      MaterialPageRoute(
        builder: (_) => WeChatChatInfoPage(
          ...
          onCreateGroupChat: () async {
            ...
            if (me == null) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(l.mirsaalIdentityHint)),
              );
              return;
            }
          ...
          canToggleCloseFriend: phone.isNotEmpty,
          muted: peer.muted,
          pinned: peer.pinned,
          hidden: peer.hidden,
          ...
          onToggleCloseFriend: (makeClose) async {
            ...
            try {
              final uri =
                  Uri.parse('${widget.baseUrl}/me/close_friends/$phone');
              final headers = await _officialHeaders(jsonBody: false);
              final resp = makeClose
          ...
          onSaveRemarksTags: (alias, tagsText) async {
            ...
            try {
              ...
              if (phone.isNotEmpty && nextTags.isNotEmpty) {
                final tags = nextTags
                    .split(',')
                    .map((e) => e.trim())
                ...
                if (tags.isNotEmpty) {
                  final uri =
                      Uri.parse('${widget.baseUrl}/me/friends/$phone/tags');
                  await http.post(
                    uri,
                ...
                child: GlassPanel(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        '${l.mirsaalContactChatIdPrefix} ${peer.id}',
                        style: theme.textTheme.bodySmall?.copyWith(
                          fontSize: 11,
                          color: theme.colorScheme.onSurface
                              .withValues(alpha: .65),
                        ...
                        children: [
                          FilterChip(
                            ...
                            onSelected: (sel) async {
                              final phone =
                                  (_friendPhones[peer.id] ?? '').trim();
                              if (phone.isEmpty) return;
                              final makeClose = sel;
                              try {
                                final uri = Uri.parse(
                                    '${widget.baseUrl}/me/close_friends/$phone');
                                final headers =
                                    await _officialHeaders(jsonBody: false);
                                final resp = makeClose
                                    ? await http.post(uri, headers: headers)
                                    : await http.delete(uri, headers: headers);
                                if (resp.statusCode >= 200 &&
                                  ...
                                  await sp.setString('friends.close',
                          ...
                          FilterChip(
                            label: Text(
                              peer.blocked ? l.mirsaalUnblock : l.mirsaalBlock,
                            ),
                      ...
                      Row(
                        children: [
                          TextButton.icon(
                            ...
                            label: Text(l.mirsaalFavoritesTitle),
                          ),
                          const SizedBox(width: 8),
                          TextButton.icon(
                      ...
                      StatefulBuilder(
                        builder: (ctx2, setThemeState) {
                          ...
                          return Wrap(
                            ...
                            children: [
                              ChoiceChip(
                                label: Text(l.mirsaalChatThemeDefault),
                              ...
                              ChoiceChip(
                                label: Text(l.mirsaalChatThemeDark),
                              ...
                              ChoiceChip(
                                label: Text(l.mirsaalChatThemeGreen),
                          ...
                          FilledButton(
                            onPressed: () async {
                              ...
                              try {
                                ...
                                // Best-effort backend sync for tags so Moments
                                // audience filters stay in sync with contact labels.
                                final phone =
                                    (_friendPhones[peer.id] ?? '').trim();
                                if (phone.isNotEmpty && tagsText.isNotEmpty) {
                                  final tags = tagsText
                                      .split(',')
                                      .map((e) => e.trim())
                                      .where((e) => e.isNotEmpty)
                                  ...
                                  if (tags.isNotEmpty) {
                                    final uri = Uri.parse(
                                        '${widget.baseUrl}/me/friends/$phone/tags');
                                    await http.post(
                                      uri,
                                      headers: await _officialHeaders(
                                          jsonBody: true),
                                      body: jsonEncode(<String, Object?>{
                                        'tags': tags,
                                      }),
                  ...
                  for (final m in _messages) {
                    ...
                    try {
                      ...
                      switch (filter) {
                        ...
                        case 'redpacket':
                          matches =
                              d.kind == 'redpacket' || d.redpacketId != null;
                          break;
                        case 'voice':
                ...
                return Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ChoiceChip(
                          label: Text(l.mirsaalSearchFilterAll),
                          selected: filter == 'all',
                          onSelected: (sel) {
                            if (!sel) return;
                            setModalState(() {
                        ...
                        ChoiceChip(
                          label: Text(l.mirsaalSearchFilterRedPackets),
                          selected: filter == 'redpacket',
                          onSelected: (sel) {
                            if (!sel) return;
                            setModalState(() {
                              filter = 'redpacket';
                        ...
                        ChoiceChip(
                          label: Text(l.mirsaalSearchFilterVoice),
                        ...
                        ChoiceChip(
                          label: Text(l.mirsaalSearchFilterCalls),
  ...
  void initState() {
    ...
    _composerFocus.addListener(() {
      ...
      if (_composerPanel == _WeChatComposerPanel.none) return;
      if (!mounted) return;
      setState(() {
        _composerPanel = _WeChatComposerPanel.none;
      });
    });
  ...
  Future<void> _bootstrap() async {
    ...
    final cachedMsgs = activePeer != null
        ...
        : <ChatMessage>[];
    // Load friend aliases, tags, pinned and recalled messages for WeChat‑style UX.
    ...
    try {
      ...
      final rawPhones = sp.getString('friends.chat_to_phone') ?? '{}';
      final decodedPhones = jsonDecode(rawPhones);
      if (decodedPhones is Map) {
        final map = <String, String>{};
        decodedPhones.forEach((k, v) {
          final key = (k ?? '').toString();
          final val = (v ?? '').toString();
          if (key.isNotEmpty && val.isNotEmpty) {
            map[key] = val;
    ...
    // Load top mini‑apps for Mirsaal hub (WeChat-style).
    unawaited(_loadTopMiniApps());
    // Best-effort: badge for service notifications (WeChat-style system thread).
  ...
  Future<void> _openPullDownMiniProgramActions(
    ...
    if (globalPos != null) {
      await _showWeChatActionsPopover(
        globalPosition: globalPos,
        actions: [
          _WeChatPopoverActionSpec(
            label: pinLabel,
  ...
  Future<void> _openSubscriptionsThread() async {
    ...
    for (final c in _contacts) {
      if (c.id == _ThreemaChatPageState._subscriptionsPeerId) {
        existing = c;
        break;
      }
    }
    ...
    if (existing == null) {
      contact = ChatContact(
        id: _ThreemaChatPageState._subscriptionsPeerId,
  ...
  Future<OfficialAccountHandle?> _loadOfficialForPeer(ChatContact peer,
    ...
    try {
      ...
      if (found != null) {
        // Try to surface a WeChat‑style welcome message in the
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      final mk = _ratchetNextSend(ratchet, peerId: _peer!.id);
  ...
  Future<void> _sendRedPacketMessage(String redpacketId, int totalAmountCents,
      int count, String message) async {
    if (_me == null || _peer == null) return;
    _sessionHash ??= _computeSessionHash();
    final me = _me!;
    final l = L10n.of(context);
    final fallbackText = message.isNotEmpty
        ? message
        : (l.isArabic ? 'هدية حمراء' : 'Red packet');
    final payload = <String, Object?>{
      "text": fallbackText,
      "kind": "redpacket",
      "redpacket_id": redpacketId,
      "redpacket_amount_cents": totalAmountCents,
  ...
  bool _handleLocalCommand(String text) {
    ...
    // /hb or /hongbao or /redpacket -> red-packet style send in current chat
    if (lower.startsWith('/hb') ||
        lower.startsWith('/hongbao') ||
        lower.startsWith('/redpacket')) {
      int? amountCents;
      final parts = t.split(RegExp(r'\s+')).where((p) => p.isNotEmpty).toList();
      for (var i = 1; i < parts.length; i++) {
        final token = parts[i];
      ...
      final peer = _peer;
      if (peer != null) {
        final initialMajor = amountCents != null ? amountCents / 100.0 : null;
        _openRedPacketComposer(
  ...
  Future<Map<String, String>> _paymentsHeaders() async {
    final sp = await SharedPreferences.getInstance();
    final cookie = sp.getString('sa_cookie') ?? '';
    final h = <String, String>{};
    if (cookie.isNotEmpty) {
  ...
  Future<void> _openRedPacketComposer(
      {double? initialAmount, required String peerId}) async {
    final me = _me;
    final peer = _peer;
    if (me == null || peer == null) return;
    final l = L10n.of(context);
            ...
            child: Container(
              ...
              child: StatefulBuilder(
                builder: (ctx2, setStateSB) {
                  ...
                  final onRed = Colors.white;
                  final onRedMuted = Colors.white.withValues(alpha: .82);
                  final peerLabel =
                      peer.name?.isNotEmpty == true ? peer.name! : peer.id;
                  return Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                          ...
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                hintText: '0.00',
                                hintStyle: TextStyle(
                                  color: onRedMuted.withValues(alpha: .7),
                                ),
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: ChoiceChip(
                              label: Text(
                                l.isArabic ? 'مبالغ متساوية' : 'Equal split',
                              ),
                              selected: !randomMode,
                              selectedColor: Colors.white,
                              labelStyle: TextStyle(
                                color:
                                    !randomMode ? Colors.red.shade700 : onRed,
                                fontWeight: !randomMode
                                    ? FontWeight.w600
                                    : FontWeight.w500,
                              ),
                              backgroundColor:
                              ...
                              onSelected: (sel) {
                                setStateSB(() {
                                  randomMode = sel ? false : randomMode;
                                });
                              },
                            ),
                          ),
                          ...
                          Expanded(
                            child: ChoiceChip(
                              label: Text(
                                l.isArabic ? 'مبالغ عشوائية' : 'Random split',
                              ),
                              selected: randomMode,
                              selectedColor: Colors.white,
                              labelStyle: TextStyle(
                                color: randomMode ? Colors.red.shade700 : onRed,
                                fontWeight: randomMode
                                    ? FontWeight.w600
                                    : FontWeight.w500,
                              ),
                              ...
                              onSelected: (sel) {
                                setStateSB(() {
                                  randomMode = sel ? true : randomMode;
                                });
                              },
                            ),
                          ),
                      ...
                      Text(
                        l.isArabic
                            ? 'قِسّم المبلغ بالتساوي أو بشكل عشوائي كما في WeChat.'
                            : 'Split the amount equally or randomly, like WeChat red packets.',
                        style: theme.textTheme.bodySmall?.copyWith(
                          fontSize: 11,
                          color: onRedMuted,
                        ),
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: TextButton(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(color: onRedMuted),
                              ),
                            ),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
                            child: PrimaryButton(
                              label: submitting
                                  ? (l.isArabic ? 'جارٍ الإرسال…' : 'Sending…')
                                  : (l.isArabic ? 'إرسال' : 'Send'),
                              onPressed: submitting
                                  ? null
                                  : () async {
                                      final amtStr = amountCtrl.text
                                          .trim()
                                          .replaceAll(',', '.');
                                      ...
                                      // Simple WeChat-like limits client-side.
                                      if (cnt > 100) {
                                        setStateSB(() {
                                          error = l.isArabic
                                      ...
                                      try {
                                        await _createRedPacket(amt, cnt,
                                            noteCtrl.text.trim(), peerId,
                      ...
                      Align(
                        ...
                        child: Text(
                          l.isArabic
                              ? 'يتم إرسال الحزمة عبر Shamell Pay.'
                              : 'Red packet is sent via Shamell Pay.',
                          style: theme.textTheme.bodySmall?.copyWith(
                            fontSize: 10,
                            color: onRedMuted,
                          ),
  ...
  Future<void> _createRedPacket(
      double amountMajor, int count, String message, String peerId,
      {required bool randomMode}) async {
    final sp = await SharedPreferences.getInstance();
    ...
    final ikey =
        'hb-${DateTime.now().millisecondsSinceEpoch.toRadixString(36)}';
    ...
    final rpId = (body['id'] ?? '').toString();
    if (rpId.isEmpty) {
      throw Exception('missing redpacket id');
    }
    final amtRaw = body['total_amount_cents'] ?? body['total_amount'];
    int totalCents;
    if (amtRaw is num) {
      totalCents = amtRaw.toInt();
    } else {
      totalCents = (amountMajor * 100).round();
    }
    final countRaw = body['total_count'];
    int totalCount;
    if (countRaw is num) {
      totalCount = countRaw.toInt();
    } else {
      totalCount = count;
    }
    await _sendRedPacketMessage(rpId, totalCents, totalCount, message);
  }
  ...
  void _maybeNotifyGroupMentions(String gid, List<ChatGroupMessage> newMsgs) {
    ...
    for (final m in newMsgs) {
      ...
      final title = (group?.name ?? '').isNotEmpty ? group!.name : gid;
      final previewBody =
      ...
      unawaited(() async {
        ...
        final l = L10n.of(context);
        final body = prefs.preview ? previewBody : l.mirsaalNewMessageBody;
  ...
  void _toggleComposerPanel(_WeChatComposerPanel panel) {
    final me = _me;
    final peer = _peer;
    ...
    setState(() {
      ...
      if (panel == _WeChatComposerPanel.more) {
      ...
      if (panel == _WeChatComposerPanel.stickers) {
        _stickersActivePackId = _stickersActivePackId.trim().isEmpty
            ? 'classic_smileys'
            : _stickersActivePackId;
        _stickersPanelFuture = _loadStickerPanelData();
      }
    });
  ...
  Future<_StickerPanelData> _loadStickerPanelData() async {
    final recent = await loadRecentStickers();
    final installed = await loadInstalledStickerPacks();

    final base = kStickerPacks.isNotEmpty ? kStickerPacks.first : null;
    final byId = <String, StickerPack>{};
    ...
    for (final pack in installed) {
      if (byId.containsKey(pack.id)) continue;
      byId[pack.id] = pack;
      ordered.add(pack);
    }

    final packBySticker = <String, String>{};
    for (final pack in ordered) {
      for (final s in pack.stickers) {
        packBySticker.putIfAbsent(s, () => pack.id);
      }
    }

    return _StickerPanelData(
      packs: ordered,
      recent: recent,
      packBySticker: packBySticker,
    );
  }

  Future<void> _sendStickerFromPanel(
    String sticker, {
    String? packId,
  }) async {
    ...
    if (packId != null && packId.trim().isNotEmpty) {
      unawaited(incrementStickerUsage(packId.trim()));
    }
    unawaited(pushRecentSticker(s));
    _msgCtrl.text = s;
    await _send();
    if (!mounted) return;
    setState(() {
  ...
  String _formatWeChatTimeHeader(DateTime dt, L10n l) {
    final d = dt.toLocal();
    final now = DateTime.now();
    ...
    if (diffDays == -1) {
      return l.isArabic ? 'أمس $time' : 'Yesterday $time';
    }
    ...
    return '${d.year}-$mon-$dd $time';
  }

  bool _shouldShowWeChatTimeHeader(int index) {
    ...
    final gap = cur.difference(prev).abs();
    return gap.inMinutes >= 5;
  }

  Widget _buildWeChatTimeHeader(DateTime dt) {
    final l = L10n.of(context);
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final label = _formatWeChatTimeHeader(dt, l);
    return Padding(
      ...
      child: Center(
        child: Container(
          ...
          decoration: BoxDecoration(
            color:
                theme.colorScheme.surface.withValues(alpha: isDark ? .35 : .55),
  ...
  Widget _buildStickerPanel() {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final future = _stickersPanelFuture ?? _loadStickerPanelData();
    _stickersPanelFuture ??= future;

    const panelHeight = 280.0;
    const recentTabId = '__recent__';
    ...
    return Container(
      ...
      child: FutureBuilder<_StickerPanelData>(
        future: future,
        builder: (context, snap) {
          ...
          final activeId = _stickersActivePackId.trim().isEmpty
              ? (hasPacks ? packs.first.id : 'classic_smileys')
              : _stickersActivePackId.trim();
          final bool isRecentTab = activeId == recentTabId;
          ...
          final stickers = () {
            if (isRecentTab) {
              return data.recent.isNotEmpty ? data.recent : fallbackStickers;
            }
            if (activePack != null && activePack.stickers.isNotEmpty) {
              return activePack.stickers;
            }
            if (hasPacks && packs.first.stickers.isNotEmpty) {
              return packs.first.stickers;
            }
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return InkWell(
                      borderRadius: BorderRadius.circular(10),
                      onTap: () {
                        final pid = data.packBySticker[s];
                        _sendStickerFromPanel(s, packId: pid);
                      },
                      child: Center(
                        child: Text(
                          s,
              ...
              Container(
                ...
                child: Row(
                  children: [
                    packButton(
                      id: recentTabId,
                      selected: isRecentTab,
                      child: const Icon(Icons.history, size: 20),
                      onTap: () {
                        setState(() {
                          _stickersActivePackId = recentTabId;
                        });
                      },
                    ),
                    Expanded(
                        ...
                        child: Row(
                          children: [
                            for (final p in packs)
                              packButton(
                                id: p.id,
                                selected: !isRecentTab && p.id == activeId,
                                child: Text(
                    ...
                    IconButton(
                      tooltip: L10n.of(context).mirsaalStickers,
                      icon: const Icon(Icons.add_circle_outline),
                      onPressed: () async {
                        await Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) =>
                                StickerStorePage(baseUrl: widget.baseUrl),
                          ),
                        );
                        if (!mounted) return;
                        setState(() {
  ...
  Widget _buildMorePanel(ChatIdentity me, ChatContact peer) {
    ...
    final actions = <({IconData icon, String label, VoidCallback onTap})>[
      ...
      (
        ...
        label: l.isArabic ? 'إرسال حزمة حمراء' : 'Red packet',
        onTap: () {
          close();
          _openRedPacketComposer(peerId: peer.id);
        },
      ),
      (
  ...
  Future<void> _openStickers() async {
    final me = _me;
    final peer = _peer;
    if (me == null || peer == null || _loading) return;
    final l = L10n.of(context);
    const storeSentinel = '__stickers_store__';
    final recent = await loadRecentStickers();
    final installedPacks = await loadInstalledStickerPacks();
    final stickers = <String>[];
    final packBySticker = <String, String>{};
    final searchMetaBySticker = <String, String>{};
    for (final pack in installedPacks) {
      final buf = StringBuffer();
      buf.write(' ');
      buf.write(pack.nameEn.toLowerCase());
      buf.write(' ');
      buf.write(pack.nameAr.toLowerCase());
      for (final tag in pack.tags) {
        buf.write(' ');
        buf.write(tag.toLowerCase());
      }
      final meta = buf.toString();
      for (final s in pack.stickers) {
        stickers.add(s);
        packBySticker[s] = pack.id;
        searchMetaBySticker[s] = meta;
      }
    }
    if (stickers.isEmpty) {
      stickers.addAll([
    ...
    bool _matchesSticker(String sticker, String query) {
      final q = query.trim().toLowerCase();
      if (q.isEmpty) return true;
      final meta = searchMetaBySticker[sticker];
      if (meta != null && meta.contains(q)) return true;
      return sticker.contains(query);
    }
          ...
          builder: (ctx, setModalState) {
            final filteredStickers = stickers.where((s) {
              if (activePackId.isNotEmpty) {
                final pid = packBySticker[s];
                if (pid != activePackId) return false;
              }
              return _matchesSticker(s, searchQuery);
            }).toList();
            final filteredRecent =
                recent.where((s) => _matchesSticker(s, searchQuery)).toList();
            return Padding(
              padding: const EdgeInsets.all(12),
              child: GlassPanel(
                padding: const EdgeInsets.all(12),
                  ...
                  children: [
                    ...
                    TextField(
                      decoration: InputDecoration(
                        ...
                        hintText: l.mirsaalStickersSearchHint,
                        isDense: true,
                    ...
                    if (installedPacks.isNotEmpty) ...[
                      ...
                      Row(
                        children: [
                          IconButton(
                            tooltip: l.mirsaalStickersRecent,
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Row(
                                children: [
                                  ChoiceChip(
                                    label: Text(
                                      l.mirsaalStickersFilterAll,
                                    ),
                                    selected: activePackId.isEmpty,
                                    onSelected: (sel) {
                                      if (!sel) return;
                                      setModalState(() {
                                        activePackId = '';
                                  ...
                                  ...installedPacks.map((pack) {
                                    final preview = pack.stickers.isNotEmpty
                                        ? pack.stickers.first
                                        : '🙂';
                                    return Padding(
                          ...
                          IconButton(
                            ...
                            icon: const Icon(Icons.add_circle_outline),
                            onPressed: () =>
                                Navigator.of(ctx).pop(storeSentinel),
                          ),
                        ],
                      ),
                    ] else
                      Align(
                        alignment: Alignment.centerRight,
                        child: TextButton(
                          onPressed: () => Navigator.of(ctx).pop(storeSentinel),
                          child: Text(
                            l.isArabic
                                ? 'المزيد من الملصقات'
                                : 'Get more stickers',
    ...
    if (selected == storeSentinel) {
      if (!mounted) return;
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => StickerStorePage(baseUrl: widget.baseUrl),
        ),
      );
      return;
    }
    final packId = packBySticker[selected];
    if (packId != null && packId.isNotEmpty) {
      // Best-effort usage tracking for sticker analytics.
      // ignore: discarded_futures
      incrementStickerUsage(packId);
    }
    // Track recently used stickers for WeChat-style recents row.
    // ignore: discarded_futures
    pushRecentSticker(selected);
    _msgCtrl.text = selected;
    await _send();
  }
  ...
  Future<void> _stopVoiceRecord() async {
    ...
    try {
      ...
      final l = L10n.of(context);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${l.mirsaalVoicePlaybackSoon} ($e)')),
      );
    }
  }
  ...
  Future<void> _forwardMessages(List<ChatMessage> msgs) async {
    ...
    try {
      _sessionHash ??= _computeSessionHash();
      final ratchet = _ensureRatchet(target);
      for (final m in msgs) {
        final decoded = _decodeMessage(m);
        ...
        if (kind != null && kind.isNotEmpty && kind != 'redpacket') {
          payload['kind'] = kind;
        }
        ...
        final mk = _ratchetNextSend(ratchet, peerId: target.id);
        final sessionKey = mk.$1;
        final keyId = mk.$2;
        final prevKeyId = mk.$3;
        final dhPubB64 = mk.$4;
        final expireAfterSeconds =
            target.disappearing && target.disappearAfter != null
                ? target.disappearAfter!.inSeconds
                : null;
  ...
  Future<void> _sendRecallForMessage(ChatMessage m) async {
    ...
    final ratchet = _ensureRatchet(peer);
    final mk = _ratchetNextSend(ratchet, peerId: peer.id);
    final sessionKey = mk.$1;
    final keyId = mk.$2;
    final prevKeyId = mk.$3;
    final dhPubB64 = mk.$4;
    final expireAfterSeconds = peer.disappearing && peer.disappearAfter != null
        ? peer.disappearAfter!.inSeconds
        : null;
    try {
      final msg = await _service.sendMessage(
        me: me,
  ...
  Future<void> _doStartVoiceRecord() async {
    try {
      ...
      if (!hasPerm) {
        final l = L10n.of(context);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.mirsaalVoicePlaybackSoon)),
        );
        return;
      }
  ...
  Future<void> _showLocalNotification() async {
    try {
      ...
      final latest = _latestCachedMessage() ??
          (_messages.isNotEmpty ? _messages.last : null);
      // If the latest message is from/to a muted contact, skip local popups
      // to mimic per-chat mute behaviour.
      OfficialNotificationMode? mode;
      String? deepLink;
      if (latest != null && _me != null) {
        final meId = _me!.id;
        final peerId = _peerIdForMessage(latest, meId);
        final senderHint = (latest.senderHint ?? '').trim();
        deepLink = Uri(
          scheme: 'shamell',
          host: 'chat',
          queryParameters: <String, String>{
            if (peerId.isNotEmpty) 'peer_id': peerId,
            if (senderHint.isNotEmpty) 'sender_hint': senderHint,
            if (latest.id.isNotEmpty) 'mid': latest.id,
          },
        ).toString();
        bool muted = false;
        ...
        try {
          ...
          if (mode == OfficialNotificationMode.muted) {
            muted = true;
          }
        } catch (_) {}
        if (muted) return;
      ...
      final usePreview = latest != null &&
          ((mode == OfficialNotificationMode.full) ||
              (mode == null && prefs.preview));
      if (usePreview) {
    ...
    try {
      ...
      if (j is Map) {
        ...
        return _DecodedPayload(
            ...
            redpacketId: (j['redpacket_id'] ?? '').toString().isEmpty
                ? null
                : (j['redpacket_id'] ?? '').toString(),
            redpacketAmountCents: j['redpacket_amount_cents'] is num
                ? (j['redpacket_amount_cents'] as num).toInt()
                : null,
            redpacketCount: j['redpacket_count'] is num
                ? (j['redpacket_count'] as num).toInt()
                : null,
            lat: lat,
            lon: lon);
      }
  ...
  String _previewText(ChatMessage m) {
    ...
    if (_recalledMessageIds.contains(m.id)) {
      return _isIncoming(m)
          ? l.mirsaalMessageRecalledByOther
          : l.mirsaalMessageRecalledByMe;
    }
    if (d.kind == 'contact') {
    ...
    if (d.kind == 'voice') {
      return l.mirsaalPreviewVoice;
    }
    if (d.kind == 'redpacket') {
      if (d.text.isNotEmpty) return d.text;
      return l.isArabic ? 'حزمة حمراء' : 'Red packet';
    }
    if (d.kind == 'location') {
      if (d.text.isNotEmpty) return d.text;
      if (d.lat != null && d.lon != null) {
        return l.mirsaalPreviewLocation;
      }
    }
            ...
            content: Column(
              ...
              children: [
                TextField(
                  ...
                  obscureText: true,
                  decoration: InputDecoration(
                      labelText: l.mirsaalBackupPassphraseLabel),
                ),
                if (confirm)
                  TextField(
                    controller: ctrl2,
                    obscureText: true,
                    decoration: InputDecoration(
                        labelText: l.mirsaalBackupPassphraseConfirm),
                  ),
              ],
            ),
            actions: [
        ...
        builder: (_) {
          return AlertDialog(
            ...
            content: TextField(
              ...
              maxLines: 3,
              decoration:
                  InputDecoration(labelText: l.mirsaalBackupDialogLabel),
            ),
            actions: [
              TextButton(
                  onPressed: () => Navigator.of(context).pop(null),
  ...
  bool _isDraftEligibleChatId(String? chatId) {
    ...
    if (id == _ThreemaChatPageState._subscriptionsPeerId) return false;
    return true;
  }
  ...
  Future<void> _stashActiveComposerDraft({bool persistNow = false}) async {
    ...
    if (!_isDraftEligibleChatId(peerId)) return;
    final id = peerId!.trim();
    _composerDraftByChatId[id] = _WeChatChatComposerDraft(
      text: _msgCtrl.text,
  ...
  Future<void> _scanQr() async {
    ...
    if (code == null) return;
    // 1) Try legacy JSON payloads (e.g. encrypted identity QR).
    try {
    ...
    // 2) Try Shamell deep links (friend / official).
    try {
      ...
      if (uri.scheme == 'shamell') {
        ...
        if (host == 'friend') {
          ...
          final idPart = segs.isNotEmpty ? segs.first.trim() : '';
          final phone = (uri.queryParameters['phone'] ?? '').trim();
          final target = phone.isNotEmpty ? phone : idPart;
          if (target.isNotEmpty) {
            await _sendFriendRequestFromQr(target);
            return;
          }
        }
        if (host == 'official') {
    ...
    // 3) Fallback: treat QR payload as plain peer id.
  ...
  Widget _buildCallHistoryRow(ChatCallLogEntry e) {
    ...
    String status;
    if (!e.accepted) {
      status = l.mirsaalCallStatusMissedShort;
    } else if (e.duration.inSeconds <= 1) {
      status = l.mirsaalCallStatusShort;
    } else {
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: Text(
            '${e.kind == 'video' ? l.mirsaalCallKindVideo : l.mirsaalCallKindVoice} • ${isOut ? l.mirsaalCallDirectionOutgoing : l.mirsaalCallDirectionIncoming} • $status',
  ...
  Widget build(BuildContext context) {
    ...
    final Color scaffoldBg = isDark
        ...
        : (isChatsList ? WeChatPalette.background : Colors.white);
    Widget body;
    switch (_tabIndex) {
      case 0:
  ...
  Widget _buildContactsTab(ChatIdentity? me, ChatContact? peer) {
    ...
    final contacts = _contacts
        .where((c) => c.id != _ThreemaChatPageState._subscriptionsPeerId)
        .where(
              ...
              children: [
                ...
                if (!filtering) ...[
                  ...
                  Column(
                    children: [
                      ListTile(
                        ...
                        dense: true,
                        title: Text(
                          l.mirsaalContactsNewFriends,
                          style: const TextStyle(fontSize: 14),
                        ),
                        subtitle: Text(
                          l.mirsaalContactsNewFriendsSubtitle,
                          style: Theme.of(context)
                              .textTheme
                              .bodySmall
                      ...
                      ListTile(
                        ...
                        dense: true,
                        title: Text(
                          l.mirsaalContactsGroups,
                          style: const TextStyle(fontSize: 14),
                        ),
                        subtitle: Text(
                          l.mirsaalContactsGroupsSubtitle,
                          style: Theme.of(context)
                              .textTheme
                              .bodySmall
                              ?.copyWith(fontSize: 12),
                      ...
                      ListTile(
                        ...
                        dense: true,
                        title: Text(
                          l.mirsaalContactsServiceAccounts,
                          style: const TextStyle(fontSize: 14),
                        ),
                        subtitle: Text(
                          l.mirsaalContactsServiceAccountsSubtitle,
                          style: Theme.of(context)
                              .textTheme
                              .bodySmall
                              ?.copyWith(fontSize: 12),
                      ...
                      ListTile(
                        ...
                        dense: true,
                        title: Text(
                          l.mirsaalContactsPeopleP2P,
                          style: const TextStyle(fontSize: 14),
                        ),
                        subtitle: Text(
                          l.mirsaalContactsPeopleP2PSubtitle,
                          style: Theme.of(context)
                              .textTheme
                              .bodySmall
                              ?.copyWith(fontSize: 12),
                        ),
                        onTap: _openPeopleP2PFromMirsaal,
                      ),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Padding(
                    ...
                    child: Text(
                      l.mirsaalContactsShamellServicesTitle,
  ...
  int _archivedThreadsCount() {
    final archivedContacts = _contacts.where((c) =>
        c.archived && c.id != _ThreemaChatPageState._subscriptionsPeerId);
    final archivedGroups =
  ...
  Widget _lastCallBanner(ChatContact peer) {
    ...
    final kindLabel =
        last.kind == 'video' ? l.mirsaalCallKindVideo : l.mirsaalCallKindVoice;
    String status;
    if (!last.accepted) {
      status = l.mirsaalCallStatusMissedShort;
    ...
    final tsLabel =
        ...
        '${ts.hour.toString().padLeft(2, '0')}:${ts.minute.toString().padLeft(2, '0')}';
    final dirLabel =
        isOut ? l.mirsaalCallDirectionOutgoing : l.mirsaalCallDirectionIncoming;
    return Padding(
  ...
  Widget _multiDeviceBanner() {
    ...
    final title = l.isArabic
        ? 'مرسال نشط أيضاً على جهاز مرتبط آخر'
        : 'Mirsaal is also active on a linked device';
  ...
  Future<void> _handleScanPayload(String raw) async {
    ...
    if (uri != null && uri.scheme.toLowerCase() == 'shamell') {
      ...
      if (host == 'friend') {
        ...
        final segs = uri.pathSegments.where((e) => e.isNotEmpty).toList();
        if ((phone == null || phone.isEmpty) && segs.isNotEmpty) {
          phone = segs.first;
        }
        if (phone != null && phone.trim().isNotEmpty) {
          await _sendFriendRequestFromQr(phone.trim());
          return;
        }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(l.mirsaalUnrecognizedQr),
      ),
    );
  }

  Future<void> _sendFriendRequestFromQr(String phone) async {
    final l = L10n.of(context);
    ...
    try {
      ...
      final headers = <String, String>{
        ...
        if (cookie.isNotEmpty) 'sa_cookie': cookie,
      };
  ...
  Widget _buildProfileTab(ChatIdentity? me) {
    ...
    final profileId = deviceId.isNotEmpty ? deviceId : null;
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => WeChatSettingsHubPage(
                        baseUrl: widget.baseUrl,
                        deviceId: deviceId,
  ...
  Widget _buildProfileHeader(ChatIdentity? me) {
    ...
    final displayName = displayNameRaw.isNotEmpty
        ? displayNameRaw
        : (l.isArabic ? 'مرسال' : 'Mirsaal');
    final idLabel = me?.id ?? l.mirsaalIdentityNotCreated;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(
                  l.isArabic ? 'معرّف مرسال: $idLabel' : 'Mirsaal ID: $idLabel',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: theme.textTheme.bodySmall?.copyWith(
                    color: theme.colorScheme.onSurface.withValues(alpha: .70),
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          // Channels, Moments & Favorites – top section like WeChat Discover
          section([
            ListTile(
              leading: Icon(
            ...
            ListTile(
              ...
              title: Text(l.mirsaalChannelMomentsTitle),
              subtitle: Text(
                l.mirsaalChannelMomentsSubtitle,
                style: theme.textTheme.bodySmall,
              ),
              trailing: const Icon(Icons.chevron_right, size: 18),
              onTap: () {
            ...
            ListTile(
              ...
              title: Text(l.mirsaalChannelFavoritesTitle),
              subtitle: Text(
                l.mirsaalChannelFavoritesSubtitle,
                style: theme.textTheme.bodySmall,
              ),
              trailing: const Icon(Icons.chevron_right, size: 18),
              onTap: () {
          ...
          const SizedBox(height: 12),
          // Official accounts & subscriptions – similar to WeChat Official Accounts
          section([
            ListTile(
              leading: Icon(
              ...
              subtitle: Text(
                l.mirsaalChannelOfficialAccountsSubtitle,
                style: theme.textTheme.bodySmall,
              ),
              trailing: Row(
                mainAxisSize: MainAxisSize.min,
            ...
            ListTile(
              ...
              title: Text(
                l.mirsaalChannelSubscriptionAccountsTitle,
              ),
              subtitle: Text(
                l.mirsaalChannelSubscriptionAccountsSubtitle,
                style: theme.textTheme.bodySmall,
              ),
              trailing: const Icon(Icons.chevron_right, size: 18),
              onTap: () {
            ...
            Wrap(
              ...
              children: _topMiniApps.map((m) {
                ...
                return ActionChip(
                  ...
                  label: Text(title),
                  onPressed: () => _openMiniAppFromMirsaal(m.id),
                );
              }).toList(),
            ),
          ],
          const SizedBox(height: 12),
          // People nearby & Shamell services – bottom section
          section([
            ListTile(
              ...
              subtitle: Text(
                l.isArabic
                    ? 'اكتشف مستخدمي وخدمات Shamell القريبة'
                    : 'Find nearby Shamell users and services',
                style: theme.textTheme.bodySmall,
              ),
              trailing: const Icon(Icons.chevron_right, size: 18),
              onTap: () {
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => WeChatShakePage(baseUrl: widget.baseUrl),
                    ),
                  );
                },
              ),
...
extension _MiniAppsFromMirsaal on _ThreemaChatPageState {
  void _openMiniAppFromMirsaal(String id) {
    unawaited(
      Navigator.push(
        context,
        MaterialPageRoute(
...
extension _MirsaalChatHelpers on _ThreemaChatPageState {
  ...
  Future<void> _onGroupLongPress(ChatGroup g, {Offset? globalPosition}) async {
    ...
    final markLabel = hasUnread ? l.mirsaalMarkRead : l.mirsaalMarkUnread;
    final pinLabel = pinned ? l.mirsaalUnpinChat : l.mirsaalPinChat;
    final moreLabel = l.isArabic ? 'المزيد' : 'More…';
    final deleteLabel = l.mirsaalDeleteChat;
    final cancelLabel = l.isArabic ? 'إلغاء' : 'Cancel';
    final globalPos = globalPosition;
    if (globalPos != null) {
      await _showWeChatActionsPopover(
        ...
        actions: [
          ...
          _WeChatPopoverActionSpec(
            ...
            onTap: () {
              unawaited(
                Future<void>.delayed(_wechatMoreSheetDelay, () {
                  if (!mounted) return;
  ...
  Future<void> _showWeChatActionsPopover({
    required Offset globalPosition,
    required List<_WeChatPopoverActionSpec> actions,
  }) async {
    ...
    Widget actionRow(
      _WeChatPopoverActionSpec action, {
  ...
  void _showChatsMenu() {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) {
        ...
        final hiddenLabel =
            _showHidden ? l.mirsaalHideLockedChats : l.mirsaalShowLockedChats;
            ...
            child: Column(
              ...
              children: [
                Builder(builder: (_) {
                  ...
                  addAction(
                    actionRow(
                      label: l.mirsaalChatsMarkAllRead,
                      onTap: () {
                        Navigator.of(ctx).pop();
                        _markAllChatsRead();
                      },
  ...
  void _openPeopleP2PFromMirsaal() {
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  // ignore: unused_element
  Future<void> _openOfficialNotificationsFromMirsaal() async {
    try {
      Perf.action('official_open_notifications_from_mirsaal_settings');
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => OfficialAccountsPage(
            ...
            Row(
              children: [
                Expanded(
                  ...
                  decoration:
                      InputDecoration(labelText: l.mirsaalDisplayNameOptional),
                )),
                const SizedBox(width: 8),
                FilledButton.icon(
          ...
          if (_chatSearch.isEmpty && !_showArchived && !_selectionMode) ...[
            if (showServiceThread)
              _buildSwipeableSystemThreadTile(
                ...
                child: _buildWeChatThreadTile(
                  leading: Stack(
                    clipBehavior: Clip.none,
                    children: [
                      _buildWeChatAvatarBox(
                ...
                child: _buildWeChatThreadTile(
                  ...
                  title: Row(
                    children: [
                      Expanded(
                        child: Text(
                          l.mirsaalSubscriptionsFeedTitle,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            fontSize: 16,
                  ...
                  subtitle: Text(
                    () {
                      ...
                      if (subs <= 0) {
                        return l.mirsaalSubscriptionsFeedEmptySummary;
                      }
                      return l.mirsaalSubscriptionsFeedSummary(subs, unread);
                    }(),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: theme.textTheme.bodySmall?.copyWith(
            ...
            Builder(builder: (ctx) {
              ...
              String groupPreview(ChatGroupMessage m) {
                ...
                if (kind == 'sealed') {
                  ...
                  previewText = l.mirsaalPreviewVoice;
                } else {
                  ...
                  if (kind == 'image' || mime.startsWith('image/')) {
                    ...
                    previewText = l.mirsaalNoMessagesYet;
                  }
                }
              ...
              Widget groupPreviewWidget(
                ...
                if (isImage) {
                  spans.add(TextSpan(text: l.mirsaalPreviewImage));
              ...
              if (threads.isEmpty) {
                ...
                if (!inArchivedView && !hasSearch) {
                  ...
                  final archivedContacts = _contacts.where((c) =>
                      c.archived &&
                      c.id != _ThreemaChatPageState._subscriptionsPeerId);
                  final archivedGroups =
                      _groups.where((g) => _archivedGroupIds.contains(g.id));
                  final archivedCount =
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (ctx2, i) {
                    ...
                    if (item.isGroup) {
                          ...
                          children: [
                            ...
                            _buildWeChatAvatarBox(
                              ...
                              child: avatarBytes != null
                                  ? Image.memory(
                                      avatarBytes,
                                      width: _wechatThreadAvatarRadius * 2,
                                      height: _wechatThreadAvatarRadius * 2,
                                      fit: BoxFit.cover,
                      ...
                      final tile = _buildWeChatThreadTile(
                        leading: leading,
                        pinned: pinned,
                        muted: muted,
                    ...
                    final isRedPacketThread = last != null &&
                        _decodeMessage(last).kind == 'redpacket';
                  ...
                  separatorBuilder: (_, __) => Divider(
                    height: 1,
                    indent: _wechatThreadDividerIndent,
                    endIndent: 0,
                  ),
  ...
  static const double _wechatSwipeActionWidth = 78.0;
  static const Duration _wechatSwipeResizeDuration =
      Duration(milliseconds: 200);
  static const Duration _wechatMoreSheetDelay = Duration(milliseconds: 90);
  static const double _wechatThreadAvatarRadius = 24.0;
  static const double _wechatThreadDividerIndent = 76.0;
  static const EdgeInsets _wechatThreadPadding =
      EdgeInsets.fromLTRB(16, 12, 12, 12);
  static const double _wechatAvatarCornerRadius = 10.0;

  Widget _buildWeChatAvatarBox({
    required Widget child,
    required Color backgroundColor,
  }) {
    final size = _wechatThreadAvatarRadius * 2;
    return ClipRRect(
      borderRadius: BorderRadius.circular(_wechatAvatarCornerRadius),
      child: Container(
        width: size,
        height: size,
  ...
  double _wechatSwipeExtentRatio(double width, int actionCount) {
    if (width <= 0) {
      return actionCount <= 2 ? 0.40 : 0.72;
    }
    return ((_wechatSwipeActionWidth * actionCount) / width).clamp(0.0, 0.92);
  }

  double _wechatSwipeSnapThreshold(double width) {
    if (width <= 0) return 0.16;
    final px = _wechatSwipeActionWidth * 0.70;
    return (px / width).clamp(0.10, 0.18);
  }

  Widget _buildWeChatThreadTile({
    ...
    final tileHeight =
        _wechatThreadAvatarRadius * 2 + _wechatThreadPadding.vertical;
  ...
  Widget _buildSwipeableSystemThreadTile({
    ...
    const wechatGray = Color(0xFFC7C7CC);
    const wechatRed = Color(0xFFFA5151);
    return LayoutBuilder(builder: (ctx, constraints) {
      final width = constraints.maxWidth;
      final extentRatio = _wechatSwipeExtentRatio(width, 2);
      final snap = _wechatSwipeSnapThreshold(width);
      Widget action({
        required String label,
        required Color bg,
  ...
  Widget _buildSwipeableGroupThreadTile(ChatGroup g, Widget child) {
    ...
    final pinned = prefs?.pinned ?? false;
    const wechatGray = Color(0xFFC7C7CC);
    const wechatOrange = Color(0xFFF7B500);
    const wechatRed = Color(0xFFFA5151);
    return LayoutBuilder(builder: (ctx, constraints) {
      final width = constraints.maxWidth;
      final extentRatio = _wechatSwipeExtentRatio(width, 3);
      final snap = _wechatSwipeSnapThreshold(width);
      Widget action({
        required String label,
        required Color bg,
        ...
        child: Column(
          ...
          children: [
            ...
            Row(children: [
              ...
              Expanded(
                  child: FilledButton.tonal(
                      ...
                      child: Text(verified
                          ? l.mirsaalVerifiedLabel
                          : l.mirsaalMarkVerifiedLabel))),
            ]),
            const SizedBox(height: 8),
            if (peer != null)
              Row(
            ...
            if (peer != null) ...[
              ...
              const SizedBox(height: 4),
              Text(
                  '${l.mirsaalYourFingerprintLabel} ${_me?.fingerprint ?? ''}'),
              const SizedBox(height: 8),
              if (_safetyNumber != null)
                Row(
                  children: [
                    Expanded(
                        child: Text(
                            '${l.mirsaalSafetyLabel} ${_safetyNumber!.formatted}',
                            style: Theme.of(context)
                                .textTheme
                                .bodySmall
  ...
  Widget _subscriptionsCard() {
    ...
    final items = _subscriptionsFilterUnreadOnly
        ...
        : _subscriptionsItems;
    return _block(
      title: l.mirsaalSubscriptionsTitle,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          if (_subscriptionsLoading)
            ...
            Align(
              ...
              child: Wrap(
                ...
                children: [
                  ChoiceChip(
                    label: Text(l.mirsaalSubscriptionsFilterAll),
                  ...
                  ChoiceChip(
                    label: Text(l.mirsaalSubscriptionsFilterUnread),
                    selected: _subscriptionsFilterUnreadOnly,
                    onSelected: (sel) {
                      if (!sel) return;
                      _applyState(() {
            ...
            Align(
              ...
              child: TextButton.icon(
                ...
                label: Text(
                  l.mirsaalSubscriptionsMarkAllRead,
                ),
              ),
            ),
            const SizedBox(height: 4),
  ...
  Widget _chatCard(ChatIdentity? me, ChatContact? peer) {
    final l = L10n.of(context);
    if (peer != null && peer.id == _ThreemaChatPageState._subscriptionsPeerId) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              child: Row(
                children: [
                  ...
                  TextButton(
                    ...
                    child: Text(
                      l.isArabic ? 'إلغاء' : l.mirsaalDialogCancel,
                    ),
                  ),
                ],
              ),
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      final showTimeHeader =
                          _shouldShowWeChatTimeHeader(i) && m.createdAt != null;
                      ...
                      if (showTimeHeader) {
                        children.add(_buildWeChatTimeHeader(m.createdAt!));
                      }
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: (_loading ||
                      ...
                      ? null
                      : () {
                          _toggleComposerPanel(_WeChatComposerPanel.stickers);
                        },
                  icon: Icon(
                    _composerPanel == _WeChatComposerPanel.stickers
                        ? Icons.keyboard_alt_outlined
                  ...
                  builder: (context, value, _) {
                    ...
                    return IconButton(
                      ...
                      onPressed: (_loading ||
                          ...
                          ? null
                          : () {
                              _toggleComposerPanel(_WeChatComposerPanel.more);
                            },
          ...
          if (_composerPanel == _WeChatComposerPanel.stickers)
            _buildStickerPanel(),
          if (_composerPanel == _WeChatComposerPanel.more &&
              me != null &&
              peer != null)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Builder(builder: (ctx) {
                      ...
                      final label =
                          '${l.mirsaalRecordingVoice} • ${_fmt(elapsed)} / ${_fmt(maxSecs)}';
  ...
  // ignore: unused_element
      ...
      actions.add({
        'icon': Icons.card_giftcard_outlined,
        'label': l.isArabic ? 'حزمة حمراء' : 'Red packet',
        'command': '/hb',
      });
    }
  ...
  Widget _bubble(BuildContext context, ChatMessage m, ChatIdentity? me) {
    ...
    final isRedPacket = decoded.kind == 'redpacket';
    ...
    if (isVoice) {
      ...
      text = l.mirsaalVoiceMessageLabel(secsLabel);
    } else if (isRedPacket) {
      text = decoded.text.isNotEmpty
          ? decoded.text
    ...
    final baseIncoming = isDark
        ...
        : Colors.white;
    const wechatOutgoing = Color(0xFF95EC69);
    ...
    final outgoingColor = chatThemeKey == 'green'
        ? (isDark ? baseOutgoing : wechatOutgoing)
        : baseOutgoing;
    ...
    final avatar = _wechatMessageAvatar(incoming: incoming, me: me);
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (isVoice && !isRecalled)
                        ...
                        else if (isRedPacket && !isRecalled)
                          InkWell(
                            onTap: () => _onRedPacketTap(m),
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(12),
                                color: theme.colorScheme.error
                                    .withValues(alpha: isDark ? .98 : .96),
                                border: Border.all(
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      if (decoded.redpacketAmountCents != null)
                                        Text(
                                          '${(decoded.redpacketAmountCents! / 100.0).toStringAsFixed(2)} SYP',
                                          style: TextStyle(
                                            fontSize: 12,
                          ...
                          _buildBubbleText(context, l.mirsaalPreviewUnknown),
                        if (showImage) ...[
  ...
  Future<void> _showWeChatMessageLongPressMenu({
    required Rect bubbleRect,
    required bool incoming,
    required List<_WeChatMessageMenuActionSpec> actions,
  ...
  Future<void> _onMessageLongPress(
    ...
    if (globalPos != null) {
      try {
        ...
        final incoming = _isIncoming(m);
        final actions = <_WeChatMessageMenuActionSpec>[];

        if (text.trim().isNotEmpty && !isVoice) {
          actions.add(
            _WeChatMessageMenuActionSpec(
          ...
          _WeChatMessageMenuActionSpec(
            ...
            onTap: () {
              unawaited(() async {
                ...
                if (!mounted) return;
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(l.mirsaalMessageFavoritedSnack)),
                );
              }());
            },
          ),
        ...
        if (d.kind == 'location' && d.lat != null && d.lon != null) {
          actions.add(
            _WeChatMessageMenuActionSpec(
              ...
              label: l.mirsaalLocationFavorite,
              onTap: () {
                unawaited(() async {
                  ...
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(l.mirsaalLocationFavoritedSnack)),
                  );
                }());
              },
            ),
        ...
        if (!isVoice && text.trim().isNotEmpty) {
          actions.add(
            _WeChatMessageMenuActionSpec(
              ...
              label: l.mirsaalTranslateMessage,
        ...
        actions.add(
          _WeChatMessageMenuActionSpec(
            ...
            label: isPinned ? l.mirsaalUnpinMessage : l.mirsaalPinMessage,
        ...
        await _showWeChatMessageLongPressMenu(
            ...
            child: Column(
              ...
              children: [
                ...
                if (text.trim().isNotEmpty && !isVoice)
                  ListTile(
                    ...
                    onTap: () async {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(l.mirsaalMessageCopiedSnack),
                        ),
                      );
                    },
                  ),
                ListTile(
                  ...
                  title: Text(l.mirsaalForwardMessage),
                  onTap: () async {
                    Navigator.of(ctx).pop();
                ...
                ListTile(
                  ...
                  title: Text(l.mirsaalAddToFavorites),
                  onTap: () async {
                    final favText =
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(l.mirsaalMessageFavoritedSnack),
                      ),
                    );
                  },
                ),
                if (d.kind == 'location' && d.lat != null && d.lon != null)
                  ListTile(
                    ...
                    title: Text(l.mirsaalLocationFavorite),
                    onTap: () async {
                      Navigator.of(ctx).pop();
                      await addFavoriteLocationQuick(
                        d.lat!,
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(l.mirsaalLocationFavoritedSnack),
                        ),
                      );
                    },
                  ),
                if (!isVoice && text.trim().isNotEmpty)
                  ListTile(
                    ...
                    title: Text(l.mirsaalTranslateMessage),
                    onTap: () async {
                      Navigator.of(ctx).pop();
                ...
                if (!isRecalled)
                  ListTile(
                    ...
                    title: Text(l.mirsaalReplyMessage),
                    onTap: () {
                      Navigator.of(ctx).pop();
                      _applyState(() {
                        _replyToMessage = m;
                ...
                ListTile(
                  ...
                  title: Text(
                      isPinned ? l.mirsaalUnpinMessage : l.mirsaalPinMessage),
                  onTap: () async {
                    Navigator.of(ctx).pop();
                ...
                if (isRecallEligible)
                  ListTile(
                    ...
                    title: Text(l.mirsaalRecallMessage),
                    onTap: () async {
                      Navigator.of(ctx).pop();
                      _recalledMessageIds.add(m.id);
                      _applyState(() {});
                ...
                if (isOwn)
                  ListTile(
                    ...
                    title: Text(l.mirsaalDeleteForMe),
                    onTap: () async {
                      Navigator.of(ctx).pop();
  ...
  Future<void> _confirmDeviceLogin(String token, {String? label}) async {
    final l = L10n.of(context);
    final theme = Theme.of(context);
    String bodyText = l.mirsaalDeviceLoginBody;
    if (label != null && label.isNotEmpty) {
      bodyText = l.isArabic
          ? 'السماح للجهاز \"$label\" بتسجيل الدخول إلى مرسال باستخدام حسابك؟'
          : 'Allow \"$label\" to sign in to Mirsaal with your account?';
    }
    final ok = await showDialog<bool>(
          context: context,
          builder: (ctx) => AlertDialog(
            title: Text(l.mirsaalDeviceLoginTitle),
            content: Text(
              bodyText,
              style: theme.textTheme.bodyMedium,
            ),
    ...
    try {
      ...
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.mirsaalDeviceLoginApprovedSnack)),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.mirsaalDeviceLoginErrorExpired)),
        );
      }
    } catch (_) {
      if (!mounted) return;
  ...
  Future<void> _onChatLongPress(ChatContact c, {Offset? globalPosition}) async {
    ...
    final markLabel = hasUnread ? l.mirsaalMarkRead : l.mirsaalMarkUnread;
    final pinLabel = c.pinned ? l.mirsaalUnpinChat : l.mirsaalPinChat;
    final moreLabel = l.isArabic ? 'المزيد' : 'More…';
    final deleteLabel = l.mirsaalDeleteChat;
    final cancelLabel = l.isArabic ? 'إلغاء' : 'Cancel';
    final globalPos = globalPosition;
            ...
            child: Wrap(
              ...
              children: [
                ...
                buildAction(
                  ...
                  l.isArabic ? 'إرسال حزمة حمراء' : 'Send red packet',
                  () {
                    Navigator.of(ctx).pop();
                    final p = _peer;
                    if (p == null) return;
  ...
  Future<Map<String, dynamic>> _fetchRedPacketStatus(String rid) async {
    final headers = await _paymentsHeaders();
    final uri = Uri.parse('${widget.baseUrl}/payments/redpacket/status/$rid');
    final resp = await http.get(uri, headers: headers);
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
  ...
  Future<Map<String, dynamic>> _claimRedPacket(String rid) async {
    final headers = await _paymentsHeaders();
    headers['content-type'] = 'application/json';
    final ikey =
        'hb-claim-${DateTime.now().millisecondsSinceEpoch.toRadixString(36)}';
    headers['Idempotency-Key'] = ikey;
    final uri = Uri.parse('${widget.baseUrl}/payments/redpacket/claim');
    final resp = await http.post(uri,
        headers: headers, body: jsonEncode({'redpacket_id': rid}));
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      String detail = 'status ${resp.statusCode}';
      try {
        final body = jsonDecode(resp.body);
  ...
  Future<void> _onRedPacketTap(ChatMessage m) async {
    final decoded = _decodeMessage(m);
    final rid = decoded.redpacketId;
    ...
    try {
      status = await _fetchRedPacketStatus(rid);
    } catch (_) {
      status = null;
    }
    if (!mounted) return;
            ...
            child: StatefulBuilder(
              builder: (ctx2, setStateSB) {
                Future<void> doClaim() async {
                  ...
                  try {
                    final c = await _claimRedPacket(rid);
                    setStateSB(() {
                      claim = c;
                    });
                  } catch (e) {
                ...
                final theme = Theme.of(ctx2);
                final title = decoded.text.isNotEmpty
                    ? decoded.text
                    : (l.isArabic ? 'حزمة حمراء' : 'Red packet');
                final statusStr =
                    (status != null ? (status!['status'] ?? '') : '')
                        .toString();
                final isFinished =
                    statusStr == 'exhausted' || statusStr == 'expired';
                final onRed = Colors.white;
                final onRedMuted = Colors.white.withValues(alpha: .80);
                final header = Column(
                  ...
                  children: [
                    ...
                    Text(
                      isFinished
                          ? (l.isArabic
                              ? 'انتهت هذه الحزمة الحمراء.'
                              : 'This red packet has finished.')
                          : (l.isArabic
                              ? 'افتح الحزمة لترى ما حصلت عليه.'
                              : 'Open the red packet to see what you get.'),
                      textAlign: TextAlign.center,
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: onRedMuted,
                        fontSize: 11,
                ...
                Widget body;
                if (claim != null) {
                  final amt = claim!['amount_cents'] is num
                      ? (claim!['amount_cents'] as num).toInt()
                      : null;
                  final amtLabel =
                      amt != null ? (amt / 100.0).toStringAsFixed(2) : '';
                  body = Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                      ...
                      Center(
                        child: Text(
                          l.isArabic ? 'لقد حصلت على' : 'You received',
                          style: theme.textTheme.bodySmall?.copyWith(
                            color: onRedMuted,
                          ),
                        ),
                  ...
                  final info = <Widget>[];
                  if (status != null) {
                    final total = status!['total_amount_cents'] is num
                        ? (status!['total_amount_cents'] as num).toInt()
                        : null;
                    final totalLabel = total != null
                        ? (total / 100.0).toStringAsFixed(2)
                        : null;
                    final claimed = status!['claimed_count'] is num
                        ? (status!['claimed_count'] as num).toInt()
                        : null;
                    final totalCount = status!['total_count'] is num
                        ? (status!['total_count'] as num).toInt()
                        : null;
                    if (totalLabel != null) {
                      info.add(
                        Text(
                          l.isArabic
                              ? 'إجمالي: $totalLabel SYP'
                              : 'Total: $totalLabel SYP',
                          style: theme.textTheme.bodyMedium?.copyWith(
                            color: onRed,
                          ),
                        ),
                    ...
                    if (claimed != null && totalCount != null) {
                    ...
                    final claimsList =
                        (status!['claims'] as List<dynamic>? ?? [])
                            .whereType<Map<String, dynamic>>()
                            .toList();
                    if (claimsList.isNotEmpty) {
                      info.add(const SizedBox(height: 8));
                      info.add(
                        Text(
                          l.isArabic ? 'النتائج' : 'Claims',
                          style: theme.textTheme.bodySmall?.copyWith(
                            fontWeight: FontWeight.w700,
                            color: onRedMuted,
                          ),
                      ...
                      for (final c in claimsList) {
                        final w = (c['wallet_id'] ?? '').toString();
                        final a = c['amount_cents'] is num
                            ? (c['amount_cents'] as num).toInt()
                            : null;
                        final idx = c['claim_index'] is num
                            ? (c['claim_index'] as num).toInt()
                            : null;
                        final short = w.isNotEmpty && w.length > 4
                            ? w.substring(w.length - 4)
                            : w;
                        final amtLabel =
                            a != null ? (a / 100.0).toStringAsFixed(2) : '';
                        info.add(
                          Text(
                            idx != null
                                ? '#$idx  •  $short  •  $amtLabel SYP'
                                : '$short  •  $amtLabel SYP',
                            style: theme.textTheme.bodySmall?.copyWith(
                              color: onRedMuted,
                            ),
                  ...
                  final canClaim = statusStr == 'active' && !claiming;
                  final claimLabel =
                      l.isArabic ? 'افتح الحزمة' : 'Open red packet';
                  body = Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                      ...
                      if (!isFinished)
                        FilledButton(
                          ...
                          onPressed: canClaim ? doClaim : null,
                          child: Text(
                            canClaim
                                ? claimLabel
                                : (l.isArabic
                      ...
                      children: [
                        ...
                        TextButton.icon(
                          ...
                          onPressed: () async {
                            ...
                            final total = status?['total_amount_cents'];
                            final totalCount = status?['total_count'];
                            if (total is num && total > 0) {
                              final major =
                                  (total.toInt() / 100.0).toStringAsFixed(2);
                              if (totalCount is num && totalCount > 0) {
                                buf.writeln(
                                  l.isArabic
                                      ? 'حزمة حمراء بقيمة $major SYP لـ ${totalCount.toInt()} مستلمين.'
                                      : 'Red packet of $major SYP for ${totalCount.toInt()} recipients.',
                                );
                              } else {
                                buf.writeln(
                                  l.isArabic
                                      ? 'حزمة حمراء بقيمة $major SYP.'
                                      : 'Red packet of $major SYP.',
                                );
                              }
                            }
                            ...
                            if (text.isEmpty) {
                              text = l.isArabic
                                  ? 'حزمة حمراء عبر Shamell Pay.'
                                  : 'Red packet via Shamell Pay.';
                            }
                            if (!text.contains('#')) {
                              text += l.isArabic
                                  ? ' #شامل_حزم_حمراء'
...
class _DecodedPayload {
  ...
  final int? redpacketAmountCents;
...
class _StickerPanelData {
  final List<StickerPack> packs;
  final List<String> recent;
  final Map<String, String> packBySticker;

  const _StickerPanelData({
    required this.packs,
    required this.recent,
    required this.packBySticker,
  });
}
...
class _WeChatPopoverActionSpec {
  final String label;
  final Color color;
  final VoidCallback onTap;

  const _WeChatPopoverActionSpec({
    required this.label,
    required this.onTap,
...
class _WeChatMessageMenuActionSpec {
  final IconData icon;
  final String label;
  final Color color;
  final VoidCallback onTap;

  const _WeChatMessageMenuActionSpec({
    required this.icon,
    required this.label,
    required this.onTap,
    this.color = Colors.white,
