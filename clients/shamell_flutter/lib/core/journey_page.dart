...
class _JourneyPageState extends State<JourneyPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l.isArabic ? 'رحلتي' : 'My journey'),
  ...
  Widget _buildRolesSection(L10n l) {
    final roles = (_home['roles'] as List?)?.map((e) => e.toString()).toList() ?? const <String>[];
    final opDomains = (_home['operator_domains'] as List?)?.map((e) => e.toString()).toList() ?? const <String>[];
    ...
    final opsText = opDomains.isEmpty ? (l.isArabic ? 'لا توجد مجالات مشغّل' : 'No operator domains') : opDomains.join(', ');
      ...
      children: [
        ...
        PrimaryButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => MobilityHistoryPage(widget.baseUrl)),
            );
          },
        ),
      ],
