...
typedef ShamellMomentsHttpGet = Future<http.Response> Function(
  Uri uri, {
  Map<String, String>? headers,
});

Uri? shamellMomentsComposerFriendsUri(String baseUrl) {
...
@visibleForTesting
Future<http.Response?> shamellMomentsComposerFetchFriendsResponse({
  ...
  final uri = shamellMomentsComposerFriendsUri(baseUrl);
  if (uri == null) {
    return null;
  }
...
double _roundShamellNominatimCoordinate(double value) =>
    (value * 10000).roundToDouble() / 10000;

@visibleForTesting
Uri? shamellMomentsReverseGeocodeUri({
  required String baseUrl,
  required double lat,
  required double lon,
}) {
  return secureApiChildUri(
    ...
    queryParameters: <String, String>{
      'lat': _roundShamellNominatimCoordinate(lat).toStringAsFixed(4),
      'lon': _roundShamellNominatimCoordinate(lon).toStringAsFixed(4),
    },
  );
}

@visibleForTesting
Uri? shamellMomentsLocationSearchUri({
  required String baseUrl,
  ...
  return secureApiChildUri(
    baseUrl: baseUrl,
    pathSegments: const <String>['me', 'geo', 'search'],
...
class ShamellMomentsComposerPage extends StatefulWidget {
  ...
  final ShamellMomentsHttpGet? friendsHttpGetForTesting;
  final ShamellMomentsHttpGet? locationHttpGetForTesting;
  final bool disableLocationAutoloadForTesting;
  ...
  const ShamellMomentsComposerPage({
    ...
    this.locationHttpGetForTesting,
    this.disableLocationAutoloadForTesting = false,
  });
    ...
    final picked = await Navigator.of(context).push<String?>(
      MaterialPageRoute(
        builder: (_) => _ShamellLocationPickerPage(
          ...
          httpGetForTesting: widget.locationHttpGetForTesting,
          disableLocationAutoloadForTesting:
              widget.disableLocationAutoloadForTesting,
        ),
      ),
    );
    if (picked == null || !mounted) return;
...
class _ShamellLocationPickerPage extends StatefulWidget {
  ...
  final ShamellMomentsHttpGet? httpGetForTesting;
  final bool disableLocationAutoloadForTesting;

  const _ShamellLocationPickerPage({
    required this.baseUrl,
    this.initialLabel,
    this.httpGetForTesting,
    this.disableLocationAutoloadForTesting = false,
  });
...
class _ShamellLocationPickerPageState
  ...
  void initState() {
    ...
    _selectedLabel = widget.initialLabel?.trim().isNotEmpty == true
        ...
        : null;
    if (!widget.disableLocationAutoloadForTesting) {
  ...
  Future<Map<String, String>> _locationHeaders(L10n l) async {
  ...
  Future<_ShamellLocationEntry?> _reverseGeocode({
    ...
    final uri = shamellMomentsReverseGeocodeUri(
        baseUrl: widget.baseUrl, lat: lat, lon: lon);
    if (uri == null) {
      throw StateError('invalid server url');
    }
    final resp =
        await _locationHttpGet(uri, headers: await _locationHeaders(l));
  ...
  Future<void> _search(
    ...
    try {
      final uri =
          shamellMomentsLocationSearchUri(baseUrl: widget.baseUrl, query: q);
      if (uri == null) {
        setState(() {
          _loadingSearch = false;
          _searchError =
  ...
  void _onQueryChanged(String v) {
    ...
    _debounce = Timer(const Duration(milliseconds: 320), () {
      if (!mounted) return;
      _search(q, generation: generation);
    });
    setState(() {});
  }

  Future<http.Response> _locationHttpGet(
    Uri uri, {
    Map<String, String>? headers,
  }) async {
    final httpGet = widget.httpGetForTesting;
    if (httpGet != null) {
      return httpGet(uri, headers: headers).timeout(
...
class _ShamellRemindPickerPageState extends State<_ShamellRemindPickerPage> {
  ...
  List<_ShamellRemindEntry> _buildEntries(
  ...
  Future<void> _loadFriends() async {
    ...
    try {
      final aliasesFuture = loadFriendAliases(baseUrlOverride: widget.baseUrl);
      http.Response? resp;
      if (widget.friendsHttpGetForTesting != null) {
        resp = await shamellMomentsComposerFetchFriendsResponse(
          baseUrl: widget.baseUrl,
      ...
      final entries = _buildEntries(list, const <String, String>{});
      if (!mounted) return;
      setState(() {
        _friends = entries;
        _loadingFriends = false;
      ...
      final enriched = _buildEntries(list, aliases);
      setState(() => _friends = enriched);
    } catch (e) {
      if (!mounted) return;
      final detail = sanitizeExceptionForUi(error: e, isArabic: l.isArabic);
      setState(() {
        _friendsError = detail ==
                (l.isArabic ? 'عنوان الخادم غير صالح.' : 'Invalid server URL.')
            ? detail
            : (l.isArabic
