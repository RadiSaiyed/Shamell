...
class _CourierMultiLevelPageState extends State<CourierMultiLevelPage> {
  ...
  bool get _isCourierOperator {
    return _roles.contains('operator_freight') || _operatorDomains.contains('freight');
  }
                ...
                children: [
                  if (_isCourierOperator)
                    Chip(
                      ...
                      backgroundColor: Tokens.colorCourierTransport.withValues(alpha: .16),
                      shape: StadiumBorder(
                        side: BorderSide(
                          color: Tokens.colorCourierTransport.withValues(alpha: .9),
                        ),
                      ),
                    ),
                  if (!_isCourierOperator)
