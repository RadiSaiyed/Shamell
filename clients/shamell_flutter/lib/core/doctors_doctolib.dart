...
class _DoctorsDoctolibPageState extends State<DoctorsDoctolibPage> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final uri = Uri.parse('${widget.baseUrl}/doctors/search').replace(queryParameters: {
      ...
      final list = j
          .map((e) => _DoctorResult(
                ...
                nextSlots: (e['next_slots'] as List?)?.map((s) => s.toString()).toList() ?? const [],
              ))
          .toList();
      setState(() {
        _results = list;
  ...
  Future<List<_Slot>> _fetchSlots(int doctorId) async {
    try {
      final uri = Uri.parse('${widget.baseUrl}/doctors/$doctorId/slots').replace(queryParameters: {
        'from_iso': DateTime.now().toIso8601String(),
        'to_iso': DateTime.now().add(const Duration(days: 7)).toIso8601String(),
      });
      final r = await http.get(uri, headers: await _hdr());
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(controller: phoneCtrl, decoration: const InputDecoration(labelText: 'Phone/email')),
                  ...
                  if (r.statusCode == 200) {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context)
                          .showSnackBar(SnackBar(content: Text('Booking failed: ${r.statusCode}')));
                    }
                  }
                } catch (e) {
                  if (mounted) {
    ...
    return GlassPanel(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          if (results.isEmpty) const Text('No doctors found')
          else ...results.map((d) => _DoctorCard(
                result: d,
                ...
                children: [
                  ...
                  Text(
                      [
                        ...
                        if (result.insurance != null && result.insurance!.isNotEmpty)
