...
class _DoctorsAdminPageState extends State<DoctorsAdminPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final toIso = DateTime.now().add(const Duration(days: 7)).toIso8601String();
      final qp = {
        'from_iso': fromIso,
        'to_iso': toIso,
      };
      ...
      final uri =
          Uri.parse('${widget.baseUrl}/admin/calendar').replace(queryParameters: qp);
  ...
  Future<void> _updateStatus(String apptId, String status) async {
    try {
      final uri = Uri.parse('${widget.baseUrl}/admin/appointments/$apptId/status');
      final r = await http.post(uri,
          headers: await _hdr(json: true), body: jsonEncode({'status': status}));
      if (r.statusCode == 200) {
        await _load();
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('Update failed: ${r.statusCode}')));
        }
      }
    } catch (e) {
      if (mounted) {
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: ListView(
                                    children: _appts
                                        .map((a) => ListTile(
                                              title: Text('${a.patient} • ${_fmt(a.tsIso)}'),
                                              subtitle: Text(
