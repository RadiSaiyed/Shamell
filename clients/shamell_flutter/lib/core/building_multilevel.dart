...
class BuildingMaterialsMultiLevelPage extends StatefulWidget {
  ...
  @override
  State<BuildingMaterialsMultiLevelPage> createState() =>
      _BuildingMaterialsMultiLevelPageState();
}

class _BuildingMaterialsMultiLevelPageState
    extends State<BuildingMaterialsMultiLevelPage> {
  bool _loading = true;
  String _error = '';
  ...
  bool get _isBuildingOperator {
    return _roles.contains('operator_commerce') ||
        _operatorDomains.contains('commerce');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final body = ListView(
      ...
      children: [
        ...
        FormSection(
          title: l.isArabic
              ? 'المستخدم النهائي (مواد البناء)'
              : 'Enduser (Building Materials)',
          subtitle: l.isArabic
          ...
          children: [
              ...
              children: [
                if (_isBuildingOperator)
                  Chip(
                    ...
                    backgroundColor:
                        Tokens.colorBuildingMaterials.withValues(alpha: .16),
                    shape: StadiumBorder(
                      side: BorderSide(
                        color:
                            Tokens.colorBuildingMaterials.withValues(alpha: .9),
                      ),
                    ),
                  ),
                if (!_isBuildingOperator)
            ...
            if (_isBuildingOperator)
              FilledButton.icon(
                ...
                label: Text(
                  l.isArabic
                      ? 'فتح مشغل مواد البناء'
                      : 'Open Building Materials operator',
                ),
              )
            else
              Text(
