...
class _BuildingCubotooPageState extends State<BuildingCubotooPage>
        ...
        if (body is List) {
          ...
          products = arr.where((p) {
            try {
              ...
              if (city.isNotEmpty && (pcity.isEmpty || !pcity.contains(city)))
                return false;
              if (cat.isNotEmpty && (pcat.isEmpty || !pcat.contains(cat)))
                return false;
              return true;
            } catch (_) {
              return true;
    ...
    if (walletId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(l.isArabic
              ? 'عيّن المحفظة أولاً'
              : 'Set your wallet first from Home')));
      return;
    }
    final priceCents = product['price_cents'] is int
        ? product['price_cents'] as int
        : int.tryParse((product['price_cents'] ?? '').toString()) ?? 0;
    final totalCents = priceCents * qty;
    setState(
        () => out = l.isArabic ? 'جارٍ إنشاء الطلب...' : 'Creating order...');
    try {
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      final params = {
        'limit': '100',
        'buyer_wallet_id': walletId,
        if (sellerWalletCtrl.text.isNotEmpty)
          'seller_wallet_id': sellerWalletCtrl.text.trim(),
      ...
      if (r.statusCode == 200) {
        ...
        if (body is List) {
          ...
          if (orderStatusFilterCtrl.text.isNotEmpty) {
            ...
            orders = arr
                .where((o) => (o['status'] ?? '').toString().toLowerCase() == s)
                .toList();
          } else {
            orders = arr;
          }
  ...
  Future<void> _attachShipment(int orderId, String shipmentId) async {
    ...
    try {
      final r = await http.post(
          Uri.parse(
              '${widget.baseUrl}/building/orders/$orderId/attach_shipment'),
  ...
  Future<void> _createMaterial() async {
    ...
    try {
      final r =
          await http.post(Uri.parse('${widget.baseUrl}/commerce/products'),
              headers: await _hdrBuilding(json: true),
              body: jsonEncode({
                'name': prodNameCtrl.text.trim(),
                'price_cents': int.tryParse(prodPriceCtrl.text.trim()) ?? 0,
                'currency': prodCurrencyCtrl.text.trim().isEmpty
                    ? 'SYP'
                    : prodCurrencyCtrl.text.trim(),
                'sku': prodSkuCtrl.text.trim().isEmpty
                    ? null
                    : prodSkuCtrl.text.trim(),
                'image_url': prodImageCtrl.text.trim().isEmpty
                    ? null
                    : prodImageCtrl.text.trim(),
                'category': prodCategoryCtrl.text.trim().isNotEmpty
                    ? prodCategoryCtrl.text.trim()
                    : null,
                'description': prodDescCtrl.text.trim().isNotEmpty
                    ? prodDescCtrl.text.trim()
                    : null,
              }));
  ...
  Future<void> _updateOrderStatus(int id, String status) async {
    ...
    try {
      final r = await http.post(
          Uri.parse('${widget.baseUrl}/building/orders/$id/status'),
          headers: await _hdrBuilding(json: true),
  ...
  Future<void> _loadOrderEvents(int orderId) async {
    try {
      final r = await http.get(
          Uri.parse('${widget.baseUrl}/building/orders/$orderId'),
          headers: await _hdrBuilding());
      if (r.statusCode == 200) {
        final body = jsonDecode(r.body);
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                Text(
                    '${(price is int ? price / 100.0 : price).toString()} $cur'),
                TextField(
                  controller: qtyCtrl,
    ...
    return GlassPanel(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(name.isEmpty ? 'Product' : name,
              style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text(
              '${price is int ? (price / 100.0).toStringAsFixed(2) : price} $cur',
      ...
      children: [
        GlassPanel(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(l.isArabic ? 'سوق مواد البناء' : 'Building Materials',
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                        onPressed: _loadProducts, child: Text(l.reSearch)),
                    if (walletId.isNotEmpty)
                      Chip(label: Text('Wallet: $walletId')),
                  ],
                ),
                if (out.isNotEmpty)
                  Padding(
                      padding: const EdgeInsets.only(top: 6), child: Text(out)),
              ],
            )),
          ...
          children: [
            ...
            Expanded(
              child: Wrap(
                ...
                runSpacing: 12,
                children: products
                    .map((p) => SizedBox(width: 260, child: _productCard(p, l)))
                    .toList(),
              ),
            ),
          ],
            ...
            Wrap(
              ...
              children: [
                ...
                SizedBox(
                    width: 100,
                    child: TextField(
                        controller: prodCurrencyCtrl,
                        decoration: const InputDecoration(labelText: 'Cur'))),
                SizedBox(
                    width: 160,
                    child: TextField(
                        controller: prodCategoryCtrl,
                        decoration:
                            const InputDecoration(labelText: 'Category'))),
                SizedBox(
                    width: 160,
                    child: TextField(
                        controller: prodSkuCtrl,
                ...
                ElevatedButton(
                    onPressed: _createMaterial, child: const Text('Create')),
              ],
            ),
            if (prodOut.isNotEmpty)
              Padding(
                  padding: const EdgeInsets.only(top: 6), child: Text(prodOut)),
          ]),
        ),
        const SizedBox(height: 12),
        Row(
          ...
          children: [
            ...
            SizedBox(
                ...
                child: TextField(
                    ...
                    decoration: const InputDecoration(
                        labelText: 'Seller wallet (optional)'))),
            SizedBox(
                width: 160,
                child: TextField(
            ...
            ElevatedButton(
                onPressed: _loadOrders, child: const Text('Refresh')),
          ],
        ),
        if (ordersLoading) const LinearProgressIndicator(minHeight: 2),
        if (ordersOut.isNotEmpty)
          Padding(
              padding: const EdgeInsets.only(top: 6), child: Text(ordersOut)),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          children: [
            ActionChip(
                label: const Text('paid_escrow'),
                onPressed: () {
                  orderStatusFilterCtrl.text = 'paid_escrow';
                  _loadOrders();
                }),
            ActionChip(
                label: const Text('shipped'),
                onPressed: () {
                  orderStatusFilterCtrl.text = 'shipped';
                  _loadOrders();
                }),
            ActionChip(
                label: const Text('delivered'),
                onPressed: () {
                  orderStatusFilterCtrl.text = 'delivered';
                  _loadOrders();
                }),
            ActionChip(
                label: const Text('disputed'),
                onPressed: () {
                  orderStatusFilterCtrl.text = 'disputed';
                  _loadOrders();
                }),
            ActionChip(
                label: const Text('all'),
                onPressed: () {
                  orderStatusFilterCtrl.clear();
                  _loadOrders();
                }),
          ],
        ),
            ...
            child: GlassPanel(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('Buyer: $buyer · Seller: $seller',
                  ...
                  Wrap(
                    ...
                    children: [
                      DropdownButton<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'paid_escrow', child: Text('paid_escrow')),
                          DropdownMenuItem(
                              value: 'shipped', child: Text('shipped')),
                          DropdownMenuItem(
                              value: 'delivered', child: Text('delivered')),
                          DropdownMenuItem(
                              value: 'disputed', child: Text('disputed')),
                          DropdownMenuItem(
                      ...
                      SizedBox(
                          ...
                          child: TextField(
                              ...
                              onSubmitted: (val) => _attachShipment(
                                  id is int
                                      ? id
                                      : int.tryParse(id.toString()) ?? 0,
                                  val))),
                      OutlinedButton(
                          onPressed: () => _attachShipment(
