...
class _ShamellContactInfoPageState extends State<ShamellContactInfoPage> {
  ...
  Future<Map<String, String>> _hdr({bool jsonBody = false}) =>
      shamellSessionHeadersForBaseUrl(widget.baseUrl, json: jsonBody);

  String _closeFriendPhone() {
    return (widget.friend['phone'] ?? widget.friend['id'] ?? '')
        .toString()
        .trim();
  }
  ...
  Future<void> _toggleCloseFriend(bool value) async {
    ...
    await _runBusy(() async {
      try {
        final uri = Uri.parse(
          '${widget.baseUrl}/me/close_friends/${Uri.encodeComponent(phone)}',
        );
        final resp = next
            ? await _http.post(uri, headers: await _hdr())
            : await _http.delete(uri, headers: await _hdr());
  ...
  void _openFriendTimeline() {
    ...
    widget.pushPage(ShamellMomentsPage(baseUrl: widget.baseUrl));
  }
