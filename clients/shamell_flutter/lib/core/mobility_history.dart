...
class _MobilityHistoryPageState extends State<MobilityHistoryPage> {
  ...
  Future<void> _loadFromCache() async {
    try {
      ...
      final tx = j['taxi'];
      final bs = j['bus'];
      final taxi = tx is List ? tx.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[];
      final bus = bs is List ? bs.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[];
  ...
  Future<void> _load({bool showSpinner = true}) async {
    ...
    try {
      ...
      final uri = Uri.parse('${widget.baseUrl}/me/mobility_history').replace(queryParameters: qp);
      final sp = await SharedPreferences.getInstance();
      ...
      if (r.statusCode == 200) {
        ...
        final tx = j['taxi'];
        final bs = j['bus'];
        _taxi = (tx is List ? tx.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[]);
        _bus = (bs is List ? bs.whereType<Map<String, dynamic>>().toList() : <Map<String, dynamic>>[]);
    ...
    final body = _loading
        ...
        : ListView(
            ...
            children: [
                        ...
                        .map(
                          (s) => DropdownMenuItem<String>(
                                ...
                                child: Text(
                                  s == 'all'
                                      ? l.statusAll
                                      : (s == 'completed' ? l.statusCompleted : l.statusCanceled),
                                ),
                              ),
                        )
                        .toList(),
              ...
              if (_taxi.isNotEmpty) ...[
                Text(L10n.of(context).homeTaxi, style: const TextStyle(fontWeight: FontWeight.w700)),
                const SizedBox(height: 4),
                ..._taxi.map(_buildTaxiTile),
                const SizedBox(height: 16),
              ],
              if (_bus.isNotEmpty) ...[
                Text(L10n.of(context).homeBus, style: const TextStyle(fontWeight: FontWeight.w700)),
                const SizedBox(height: 4),
              ...
              if (_taxi.isEmpty && _bus.isEmpty && _out.isEmpty)
                Padding(
                  ...
                  child: Text(
                    l.noMobilityHistory,
                    style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: .70)),
                  ),
                ),
              if (_out.isNotEmpty)
                Padding(
                  padding: const EdgeInsets.only(top: 12),
                  child: Text(_out, style: TextStyle(color: Theme.of(context).colorScheme.error)),
                ),
            ],
          );
    return Scaffold(
  ...
  Widget _buildTaxiTile(Map<String, dynamic> r) {
    ...
    return StandardListTile(
      ...
      subtitle: subtitle.isEmpty ? null : Text(subtitle.toString(), style: const TextStyle(fontSize: 12)),
    );
  }
