...
import '../main.dart' show WaterButton, AppBG;
...
class EquipmentOpsDashboardPage extends StatefulWidget {
  ...
  @override
  State<EquipmentOpsDashboardPage> createState() => _EquipmentOpsDashboardPageState();
}
...
class _EquipmentOpsDashboardPageState extends State<EquipmentOpsDashboardPage> {
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      final b = await http.get(Uri.parse('$_base/equipment/bookings?status=active&limit=100'));
      if (b.statusCode == 200) {
      ...
      for (final x in _bookings) {
        ...
        if (tasks.isNotEmpty) {
          final deliveryTask = tasks.firstWhere((t) => (t as Map?)?['kind'] == 'delivery',
              orElse: () => tasks.first);
  ...
  Future<void> _loadCalendar() async {
    ...
    try {
      final monthStr = '${_month.year.toString().padLeft(4, '0')}-${_month.month.toString().padLeft(2, '0')}';
      final uri = Uri.parse('$_base/equipment/ops/calendar').replace(queryParameters: {'month': monthStr});
      final r = await http.get(uri);
      if (r.statusCode == 200) {
        final body = jsonDecode(r.body);
        if (body is Map && body['events'] is List) {
          _calEvents = body['events'] as List<dynamic>;
          _holds = _calEvents.where((e) => e is Map && e['type'] == 'hold').toList();
        }
      } else {
  ...
  Widget _map() {
    ...
    for (final b in _bookings) {
      ...
      if (tasks.isNotEmpty) {
        deliveryTask = tasks.cast<Map<String, dynamic>>().firstWhere((t) => t['kind'] == 'delivery',
    ...
    return SizedBox(
      height: 260,
      child: TaxiMapWidget(center: center, initialZoom: 10, markers: markers, polylines: polylines),
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        ...events.map((e) {
          ...
          final header = isHold ? 'Hold / ${m['reason'] ?? ''}' : 'Booking ${m['status'] ?? ''}';
          return Glass(
            ...
            child: Column(
              ...
              children: [
                ...
                if (!isHold && m['project'] != null)
                  Text('Projekt: ${m['project']}', style: const TextStyle(fontSize: 12)),
                if (!isHold && m['po'] != null)
                  Text('PO: ${m['po']}', style: const TextStyle(fontSize: 12)),
                if (!isHold && m['attachments'] is List && (m['attachments'] as List).isNotEmpty)
                  Text('Attachments: ${(m['attachments'] as List).join(", ")}',
                      style: const TextStyle(fontSize: 12)),
                if (!isHold && m['signature_name'] != null)
                  Text('POD: ${m['signature_name']}', style: const TextStyle(fontSize: 12)),
              ],
            ),
          );
        }),
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Glass(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Eq ${m['equipment_id']}', style: const TextStyle(fontWeight: FontWeight.w700)),
                            Text(m['delivery_address']?.toString() ?? '', style: const TextStyle(fontSize: 12)),
                            Text(m['from_iso']?.toString() ?? '', style: const TextStyle(fontSize: 12)),
                            Row(
                              children: [
                                TextButton(onPressed: () => _openAssignSheet(m), child: const Text('Assign')),
                              ],
                            )
                          ],
                        ),
  ...
  Future<void> _openTaskStatus(Map<String, dynamic> booking, String status) async {
    try {
      final body = [
  ...
  Future<void> _openAssignSheet(Map<String, dynamic> booking) async {
    final driver = TextEditingController(text: booking['driver_name']?.toString() ?? '');
    final phone = TextEditingController(text: booking['driver_phone']?.toString() ?? '');
    final eta = TextEditingController(text: booking['eta_minutes']?.toString() ?? '');
            ...
            child: StatefulBuilder(builder: (context, setSheet) {
                  ...
                  final body = [
                    {
                      ...
                      'window_start_iso': wStart.text.trim().isEmpty ? null : wStart.text.trim(),
                      'window_end_iso': wEnd.text.trim().isEmpty ? null : wEnd.text.trim(),
                      'proof_photo_url': proofUrl.text.trim().isEmpty ? null : proofUrl.text.trim(),
                      'proof_note': proofNote.text.trim().isEmpty ? null : proofNote.text.trim(),
                      'signature_name': signature.text.trim().isEmpty ? null : signature.text.trim(),
                    }
                  ];
                  final r = await http.post(
                      Uri.parse('$_base/equipment/bookings/${booking['id']}/logistics'),
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text('Assign delivery', style: Theme.of(context).textTheme.titleLarge),
                    ...
                    TextField(controller: eta, decoration: const InputDecoration(labelText: 'ETA minutes')),
                    TextField(controller: wStart, decoration: const InputDecoration(labelText: 'Window start (ISO)')),
                    TextField(controller: wEnd, decoration: const InputDecoration(labelText: 'Window end (ISO)')),
                    TextField(controller: proofUrl, decoration: const InputDecoration(labelText: 'Proof photo URL (optional)')),
                    TextField(controller: proofNote, decoration: const InputDecoration(labelText: 'Proof note')),
                    TextField(controller: signature, decoration: const InputDecoration(labelText: 'Signature name')),
      ...
      body: Stack(
        children: [
          ...
          SafeArea(
            ...
            const Text('Active deliveries', style: TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            _boardView(),
            const SizedBox(height: 8),
            _map(),
            ...
            ..._bookings.map((b) {
                  ...
                  return GlassPanel(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Eq ${m['equipment_id']} • ${m['status']}', style: const TextStyle(fontWeight: FontWeight.w700)),
                        Text('From: ${m['from_iso'] ?? ''}'),
                        Text('To: ${m['to_iso'] ?? ''}'),
                        if (m['delivery_address'] != null) Text('Delivery: ${m['delivery_address']}'),
                        if (m['pickup_address'] != null) Text('Pickup: ${m['pickup_address']}'),
                        if (m['project'] != null) Text('Projekt: ${m['project']}', style: const TextStyle(fontSize: 12)),
                        if (m['po'] != null) Text('PO: ${m['po']}', style: const TextStyle(fontSize: 12)),
                        if (m['attachments'] is List && (m['attachments'] as List).isNotEmpty)
                          Text('Attachments: ${(m['attachments'] as List).join(", ")}',
                              style: const TextStyle(fontSize: 12)),
                        if (m['insurance'] == true) const Text('Versicherung aktiv', style: TextStyle(fontSize: 12)),
                        if (m['damage_waiver'] == true)
                          const Text('Schadenverzicht aktiv', style: TextStyle(fontSize: 12)),
                        if (m['delivery_distance_km'] != null) Text('Route: ${m['delivery_distance_km']} km'),
                        if (m['tasks'] is List)
                            ...
                            return Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (t['eta_minutes'] != null) Text('ETA: ${t['eta_minutes']} min', style: const TextStyle(fontSize: 12)),
                                  if (t['driver_name'] != null) Text('Driver: ${t['driver_name']} (${t['driver_phone'] ?? ''})', style: const TextStyle(fontSize: 12)),
                                  if (t['window_start_iso'] != null)
                                    Text('Window: ${t['window_start_iso']} - ${t['window_end_iso'] ?? ''}', style: const TextStyle(fontSize: 12)),
                                  if (t['arrived_iso'] != null) Text('Arrived: ${t['arrived_iso']}', style: const TextStyle(fontSize: 12)),
                                  if (t['completed_iso'] != null) Text('Completed: ${t['completed_iso']}', style: const TextStyle(fontSize: 12)),
                                  if (t['proof_photo_url'] != null) Text('Proof: ${t['proof_photo_url']}', style: const TextStyle(fontSize: 12)),
                                  if (t['proof_note'] != null) Text('Notiz: ${t['proof_note']}', style: const TextStyle(fontSize: 12)),
                                  if (t['signature_name'] != null) Text('Signature: ${t['signature_name']}', style: const TextStyle(fontSize: 12)),
                                ],
                              ),
                            );
                          }).toList(),
                        if (m['invoice'] is Map && (m['invoice'] as Map)['pdf_url'] != null)
                          Padding(
                            padding: const EdgeInsets.only(top: 6),
                            child: Text('Rechnung: ${(m['invoice'] as Map)['status']} • ${(m['invoice'] as Map)['pdf_url']}',
                                style: const TextStyle(fontSize: 12)),
                          ),
                        Row(
                          children: [
                            TextButton(onPressed: () => _openAssignSheet(m), child: const Text('Assign/ETA')),
                            const SizedBox(width: 8),
                            TextButton(onPressed: () => _openTaskStatus(m, 'completed'), child: const Text('Mark delivered')),
                            const SizedBox(width: 8),
                            TextButton(
