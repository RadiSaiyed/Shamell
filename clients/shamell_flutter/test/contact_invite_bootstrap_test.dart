...
void main() {
  ...
  test('redeemContactInviteToken bootstraps chat device when missing',
    ...
    expect(me, isNotNull);
    final tok = await ChatLocalStore().loadDeviceAuthToken(
      me!.id,
      baseUrlOverride: 'http://127.0.0.1:8080',
    );
    expect(tok, 'tok-1');
  ...
  test('createContactInviteToken bootstraps chat device and forwards header',
      () async {
    final calls = <String>[];
    String? registeredDeviceId;
    String? inviteHeaderDeviceId;
    ...
    final mock = MockClient((req) async {
      ...
      if (req.url.path == '/contacts/invites') {
        inviteHeaderDeviceId =
            req.headers['X-Chat-Device-Id'] ?? req.headers['x-chat-device-id'];
        final body = jsonDecode(req.body) as Map<String, Object?>;
        requestedMaxUses = (body['max_uses'] as num?)?.toInt();
        return http.Response(
          jsonEncode({
            'token': 'a' * 64,
    ...
    final svc = ChatService('http://127.0.0.1:8080', httpClient: mock);
    final token = await svc.createContactInviteToken(maxUses: 3);

    expect(token, 'a' * 64);
    expect(registeredDeviceId, isNotNull);
    expect(inviteHeaderDeviceId, registeredDeviceId);
    expect(requestedMaxUses, 3);
    expect(calls.first, 'POST /chat/devices/register');
    expect(calls.last, 'POST /contacts/invites');
  });

  test('createContactInviteToken does not re-bootstrap on subsequent calls',
      () async {
    final calls = <String>[];
    var createCount = 0;
    ...
    final mock = MockClient((req) async {
      ...
      if (req.url.path == '/contacts/invites') {
        ...
        return http.Response(
          jsonEncode({
            'token': '${createCount.toRadixString(16).padLeft(64, 'a')}',
            'expires_at': '2026-03-16T00:00:00Z',
            'max_uses': 1,
          }),
          200,
    ...
    final svc = ChatService('http://127.0.0.1:8080', httpClient: mock);
    await svc.createContactInviteToken(maxUses: 1);
    final firstLen = calls.length;
    await svc.createContactInviteToken(maxUses: 1);
    final delta = calls.sublist(firstLen);

    expect(delta, <String>['POST /contacts/invites']);
  });

  test(
      'createContactInviteToken retries ownership failure without dropping x-chat-device-id',
      () async {
    final registerDeviceIds = <String>[];
    final inviteHeaderDeviceIds = <String?>[];
    var inviteCalls = 0;
    ...
    final svc = ChatService('http://127.0.0.1:8080', httpClient: mock);
    final token = await svc.createContactInviteToken(maxUses: 1);

    expect(token, 'b' * 64);
    expect(registerDeviceIds.length, 2);
    expect(inviteHeaderDeviceIds.length, 2);
    expect(inviteHeaderDeviceIds[0], registerDeviceIds.first);
    expect(inviteHeaderDeviceIds[1], registerDeviceIds.last);
    expect(inviteHeaderDeviceIds[0], isNotEmpty);
    expect(inviteHeaderDeviceIds[1], isNotEmpty);
  });
}
