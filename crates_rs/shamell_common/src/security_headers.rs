...
fn default_csp_header_value() -> &'static str {
    "default-src 'none'; base-uri 'none'; frame-ancestors 'none'; object-src 'none'; form-action 'self'; img-src 'self' data:; style-src 'self'; script-src 'self'; connect-src 'self'; manifest-src 'self'; frame-src 'none'"
}
...
mod tests {
    ...
    #[tokio::test]
    async fn from_env_invalid_hsts_value_falls_back_to_environment_default() {
        let layer = {
            let _g = ENV_LOCK.get_or_init(|| Mutex::new(())).lock().unwrap();
            let _env = EnvGuard::new(&[
                "SECURITY_HEADERS_ENABLED",
            ...
            env::remove_var("SECURITY_HEADERS_ENABLED");
            env::set_var("HSTS_ENABLED", "hsts-ish");
            env::remove_var("CSP_ENABLED");
            env::remove_var("CSP_HEADER_VALUE");
            SecurityHeadersLayer::from_env("dev")
        };
        ...
        assert!(
            ...
            "invalid HSTS_ENABLED must not flip dev default to enabled"
        );
    }
    ...
    #[test]
    fn default_csp_rejects_unsafe_inline_and_broad_network_sources() {
        let csp = default_csp_header_value();
