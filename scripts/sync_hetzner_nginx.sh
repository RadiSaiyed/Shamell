...
WELL_KNOWN_DIR="${REPO_ROOT}/ops/hetzner/www/online.shamell.online/.well-known"
CF_REALIP_SNIPPET="${CLOUDFLARE_REALIP_SNIPPET:-${REPO_ROOT}/ops/hetzner/nginx/snippets/shamell_cloudflare_realip.conf}"
...
ensure_fresh_cloudflare_realip_snippet() {
  CLOUDFLARE_REALIP_SNIPPET="${CF_REALIP_SNIPPET}" \
    bash "${REPO_ROOT}/scripts/check_cloudflare_realip_freshness.sh"
}
...
if [[ -d "$WELL_KNOWN_DIR" ]]; then
  echo "Copying mobile well-known assets to ${HOST_ALIAS}:${tmp_remote}/well-known"
  ssh "$HOST_ALIAS" "mkdir -p '$tmp_remote/well-known'"
  scp "$WELL_KNOWN_DIR"/* "${HOST_ALIAS}:${tmp_remote}/well-known/" || true
fi
...
sudo_run install -d -m 0755 /etc/nginx/snippets
sudo_run install -d -m 0755 /etc/nginx/conf.d
sudo_run install -d -m 0755 /var/www/shamell
sudo_run install -d -m 0755 /var/www/shamell/.well-known

if [[ -f '${tmp_remote}/conf.d/shamell_log_formats.conf' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/conf.d/shamell_log_formats.conf' /etc/nginx/conf.d/shamell_log_formats.conf
fi

if [[ -f '${tmp_remote}/snippets/shamell_cloudflare_realip.conf' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/snippets/shamell_cloudflare_realip.conf' /etc/nginx/snippets/shamell_cloudflare_realip.conf
fi
if [[ -f '${tmp_remote}/snippets/shamell_bff_edge_hardening.conf' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/snippets/shamell_bff_edge_hardening.conf' /etc/nginx/snippets/shamell_bff_edge_hardening.conf
fi
if [[ -f '${tmp_remote}/snippets/shamell_bff_internal_auth.local.conf' ]]; then
  sudo_run install -m 0600 '${tmp_remote}/snippets/shamell_bff_internal_auth.local.conf' /etc/nginx/snippets/shamell_bff_internal_auth.local.conf
fi
...
if ! sudo_run grep -qE 'proxy_set_header[[:space:]]+X-Internal-Secret[[:space:]]+\".+\";' /etc/nginx/snippets/shamell_bff_internal_auth.local.conf; then
...
if [[ ! -f /etc/nginx/snippets/shamell_bff_role_attestation.local.conf ]] && [[ -f '${tmp_remote}/snippets/shamell_bff_role_attestation.local.conf.example' ]]; then
  sudo_run install -m 0600 '${tmp_remote}/snippets/shamell_bff_role_attestation.local.conf.example' /etc/nginx/snippets/shamell_bff_role_attestation.local.conf
fi
if [[ ! -f /etc/nginx/snippets/shamell_dashboard_no_login.local.conf ]] && [[ -f '${tmp_remote}/snippets/shamell_dashboard_no_login.local.conf.example' ]]; then
  sudo_run install -m 0600 '${tmp_remote}/snippets/shamell_dashboard_no_login.local.conf.example' /etc/nginx/snippets/shamell_dashboard_no_login.local.conf
fi
if [[ ! -f /etc/nginx/snippets/shamell_dashboard_no_login_allowlist.local.conf ]] && [[ -f '${tmp_remote}/snippets/shamell_dashboard_no_login_allowlist.local.conf.example' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/snippets/shamell_dashboard_no_login_allowlist.local.conf.example' /etc/nginx/snippets/shamell_dashboard_no_login_allowlist.local.conf
fi
if [[ -f '${tmp_remote}/snippets/shamell_docs_allowlist.local.conf' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/snippets/shamell_docs_allowlist.local.conf' /etc/nginx/snippets/shamell_docs_allowlist.local.conf
  ...
  sudo_run install -m 0644 '${tmp_remote}/snippets/shamell_docs_allowlist.local.conf.example' /etc/nginx/snippets/shamell_docs_allowlist.local.conf
fi

sudo_run install -m 0644 '${tmp_remote}/api.shamell.online' /etc/nginx/sites-available/api.shamell.online
if [[ -f '${tmp_remote}/staging-api.shamell.online' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/staging-api.shamell.online' /etc/nginx/sites-available/staging-api.shamell.online
fi
if [[ -f '${tmp_remote}/livekit.shamell.online' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/livekit.shamell.online' /etc/nginx/sites-available/livekit.shamell.online
fi
sudo_run install -m 0644 '${tmp_remote}/media.shamell.online' /etc/nginx/sites-available/media.shamell.online
sudo_run install -m 0644 '${tmp_remote}/online.shamell.online' /etc/nginx/sites-available/online.shamell.online
sudo_run install -m 0644 '${tmp_remote}/shamell.online' /etc/nginx/sites-available/shamell.online

if [[ -f '${tmp_remote}/well-known/apple-app-site-association' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/well-known/apple-app-site-association' /var/www/shamell/.well-known/apple-app-site-association
else
  echo 'WARNING: missing apple-app-site-association; iOS Universal Links will not verify.' >&2
fi
if [[ -f '${tmp_remote}/well-known/assetlinks.json' ]]; then
  sudo_run install -m 0644 '${tmp_remote}/well-known/assetlinks.json' /var/www/shamell/.well-known/assetlinks.json
else
  echo 'WARNING: missing assetlinks.json; Android App Links will not verify.' >&2
fi

sudo_run ln -sfn /etc/nginx/sites-available/api.shamell.online /etc/nginx/sites-enabled/api.shamell.online
if [[ -f /etc/nginx/sites-available/staging-api.shamell.online ]]; then
  sudo_run ln -sfn /etc/nginx/sites-available/staging-api.shamell.online /etc/nginx/sites-enabled/staging-api.shamell.online
fi
if [[ -f /etc/nginx/sites-available/livekit.shamell.online ]]; then
  if [[ -f /etc/letsencrypt/live/livekit.shamell.online/fullchain.pem ]] && [[ -f /etc/letsencrypt/live/livekit.shamell.online/privkey.pem ]]; then
    sudo_run ln -sfn /etc/nginx/sites-available/livekit.shamell.online /etc/nginx/sites-enabled/livekit.shamell.online
  else
    sudo_run rm -f /etc/nginx/sites-enabled/livekit.shamell.online
  fi
fi
sudo_run ln -sfn /etc/nginx/sites-available/media.shamell.online /etc/nginx/sites-enabled/media.shamell.online
sudo_run ln -sfn /etc/nginx/sites-available/online.shamell.online /etc/nginx/sites-enabled/online.shamell.online
sudo_run ln -sfn /etc/nginx/sites-available/shamell.online /etc/nginx/sites-enabled/shamell.online
