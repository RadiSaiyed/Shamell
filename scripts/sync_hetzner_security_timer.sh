...
scp \
  "${REPO_ROOT}/scripts/security_events_report.sh" \
  "${REPO_ROOT}/scripts/security_alert_webhook_drill.sh" \
  "${REPO_ROOT}/scripts/lib_internal_identity.sh" \
...
ssh -tt "$HOST_ALIAS" "
  ...
  install -m 0644 '${tmp_remote}/lib_internal_identity.sh' '${REMOTE_APP_DIR}/scripts/lib_internal_identity.sh'
...
if [[ "${RUN_DRILL}" == "1" ]]; then
  ...
  if [[ -n "${SECURITY_ALERT_DRILL_ALERTS:-}" ]]; then
    remote_drill_env+="SECURITY_ALERT_DRILL_ALERTS=$(quote_for_remote "$SECURITY_ALERT_DRILL_ALERTS") "
  fi
  if [[ -n "${SECURITY_ALERT_DRILL_SEVERITY:-}" ]]; then
    remote_drill_env+="SECURITY_ALERT_DRILL_SEVERITY=$(quote_for_remote "$SECURITY_ALERT_DRILL_SEVERITY") "
  fi
  if [[ -n "${SECURITY_ALERT_DRILL_SERVICE:-}" ]]; then
    remote_drill_env+="SECURITY_ALERT_DRILL_SERVICE=$(quote_for_remote "$SECURITY_ALERT_DRILL_SERVICE") "
  fi
  if [[ -n "${SECURITY_ALERT_DRILL_NOTE:-}" ]]; then
    remote_drill_env+="SECURITY_ALERT_DRILL_NOTE=$(quote_for_remote "$SECURITY_ALERT_DRILL_NOTE") "
  fi
  ssh -tt "$HOST_ALIAS" "
    set -euo pipefail
    cd '${REMOTE_APP_DIR}'
    ${remote_drill_env}ENV_FILE='${REMOTE_ENV_FILE}' ./scripts/security_alert_webhook_drill.sh
  "
fi
