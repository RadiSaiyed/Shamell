...
LOG_FILE="${SECURITY_ALERT_LOG_FILE:-}"
...
THRESHOLDS="${SECURITY_ALERT_THRESHOLDS:-device_login_approve.blocked:5,device_login_redeem.blocked:5,biometric_login.blocked:10,biometric_login_attestation.revoked:1,biometric_token_rotate.failed:1,auth_rate_limit_exceeded.blocked:30,admin_role_mutation.blocked:1,admin_role_mutation.failed:1,ride_sensitive_mutation.blocked:1,ride_sensitive_mutation.failed:1,contact_invite_create_chat_device_header_missing.blocked:5,chat_protocol_downgrade.blocked:1,chat_key_bundle_policy.blocked:1,chat_key_register_policy.blocked:1,chat_key_bootstrap_policy.blocked:1,chat_prekey_inventory.low:3}"
...
WEBHOOK_URL_ENV_SET=0
if [[ "${SECURITY_ALERT_WEBHOOK_URL+x}" == x ]]; then
  WEBHOOK_URL_ENV_SET=1
fi

usage() {
  cat <<'USAGE'
Usage: scripts/security_events_report.sh [--log-file <path>|-]

Summarizes recent security-event logs and evaluates threshold-based alerts.
Default source is docker compose logs for SECURITY_ALERT_SERVICE.

Options:
  --log-file <path>  replay JSON logs from file instead of docker compose logs
  --log-file -       read JSON logs from stdin instead of docker compose logs
  --help, -h         show this help

Environment overrides:
  SECURITY_ALERT_LOG_FILE  same as --log-file
USAGE
}
...
if (( WEBHOOK_URL_ENV_SET == 0 )) && [[ -z "$WEBHOOK_URL" ]] && [[ -n "$LOG_FILE" ]]; then
  :
else
...
WEBHOOK_SIGNING_SEED_B64="$(value_or_env_file SECURITY_ALERT_WEBHOOK_SIGNING_SEED_B64 "$WEBHOOK_SIGNING_SEED_B64")"
WEBHOOK_AUDIENCE="$(value_or_env_file SECURITY_ALERT_WEBHOOK_AUDIENCE "$WEBHOOK_AUDIENCE")"
...
state_updates_file="${tmp_dir}/state_updates.tsv"
...
read_security_logs() {
  if [[ -n "$LOG_FILE" ]]; then
    ...
    if [[ ! -f "$LOG_FILE" ]]; then
      echo "SECURITY_ALERT_LOG_FILE not found: $LOG_FILE" >&2
      return 1
    fi
    cat "$LOG_FILE"
    return 0
...
read_security_logs | parse_security_events_tsv > "$events_file"
...
if [[ -s "$alerts_file" ]]; then
  while IFS= read -r alert; do
    ...
    printf "%s\t%s\n" "$key" "$now_epoch" >> "$state_updates_file"
  done < "$alerts_file"
fi
...
if [[ -n "$LOG_FILE" && -z "$WEBHOOK_URL" ]]; then
  echo "Replay mode active; webhook delivery disabled unless SECURITY_ALERT_WEBHOOK_URL is explicitly set."
fi
...
if [[ -n "$WEBHOOK_URL" && -s "$unsuppressed_file" ]]; then
  ...
  payload_file="${tmp_dir}/webhook-payload.json"
...
if [[ -s "$state_updates_file" ]]; then
  while IFS=$'\t' read -r key sent_at; do
    [[ -z "$key" ]] && continue
    awk -F'\t' -v k="$key" '$1 != k' "$state_work_file" > "${state_work_file}.tmp"
    mv "${state_work_file}.tmp" "$state_work_file"
    printf "%s\t%s\n" "$key" "$sent_at" >> "$state_work_file"
  done < "$state_updates_file"
fi
