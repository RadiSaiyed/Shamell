...
log "subscribing B to /payments/events/stream before transfer to assert realtime delivery"
SSE_OUT="$LOG_DIR/sse_b.log"
SSE_TIMEOUT_SECS=10
: >"$SSE_OUT"
(
  curl -sS -N --max-time "$SSE_TIMEOUT_SECS" \
    -H "Accept: text/event-stream" \
    -H "Cookie: ${COOKIE_B}" \
    -H "x-device-id: ${DEVICE_B}" \
    "${BASE_URL}/payments/events/stream" >>"$SSE_OUT" 2>&1
) &
SSE_PID=$!
# Give the stream a moment to attach to the broadcast channel before we fire.
sleep 1
...
XFER_TXN_ID="$(jq -r '.txn_id // empty' "$XFER_OUT")"
[[ -n "$XFER_TXN_ID" ]] || fail "transfer response missing txn_id; got $(cat "$XFER_OUT")"

# Wait up to SSE_TIMEOUT_SECS for the streamed event to land in $SSE_OUT.
SSE_DEADLINE=$(( $(date +%s) + SSE_TIMEOUT_SECS ))
while (( $(date +%s) < SSE_DEADLINE )); do
  if grep -q "\"event_type\":\"transfer.credit\"" "$SSE_OUT" \
     && grep -q "\"txn_id\":\"$XFER_TXN_ID\"" "$SSE_OUT"; then
    break
  fi
  sleep 0.2
done
...
grep -q "\"event_type\":\"transfer.credit\"" "$SSE_OUT" \
  || fail "realtime SSE did not deliver transfer.credit to B (log: $SSE_OUT)"
grep -q "\"txn_id\":\"$XFER_TXN_ID\"" "$SSE_OUT" \
  || fail "realtime SSE event missing matching txn_id=$XFER_TXN_ID (log: $SSE_OUT)"
SSE_KEEPALIVES="$(grep -c '^: keepalive' "$SSE_OUT" || true)"
log "realtime: transfer.credit delivered via SSE (keepalives=$SSE_KEEPALIVES)"
...
API checks:
  ...
  realtime_sse_credit(B): ok (txn=$XFER_TXN_ID, keepalives=$SSE_KEEPALIVES)
